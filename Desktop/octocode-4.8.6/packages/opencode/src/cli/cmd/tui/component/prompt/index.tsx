...
import { EmptyBorder, SplitBorder } from "@/cli/cmd/tui/component/border"
import { DialogVariant, VARIANT_COLORS } from "@/cli/cmd/tui/component/dialog-variant"
import { DialogModel } from "@/cli/cmd/tui/component/dialog-model"
import { NavItem, MemoryButton } from "@/cli/cmd/tui/routes/home"
import { useSDK } from "@/cli/cmd/tui/context/sdk"
import { useRoute } from "@/cli/cmd/tui/context/route"
...
import * as Editor from "@/cli/cmd/tui/util/editor"
import * as Voice from "@/cli/cmd/tui/util/voice"
import { useExit } from "../../context/exit"
import * as Clipboard from "../../util/clipboard"
import type { AssistantMessage, FilePart, UserMessage } from "@octo-ai/sdk/v2"
...
export function Prompt(props: PromptProps) {
  ...
  const showVariant = createMemo(() => {
    return !!local.agent.current() && store.mode === "normal"
  })
                ...
                <Show when={store.mode === "normal" && local.agent.current()}>
                  ...
                  <Show when={showVariant()}>
                    <box
                      ...
                      paddingRight={1}
                      onMouseUp={() => {
                        const pos = { x: dimensions().width - 22, y: dimensions().height - 6 }
                        dialog.replace(() => <DialogVariant />, undefined, pos)
                      }}
                    >
                      <text fg={fadeColor(VARIANT_COLORS[local.model.variant.current() || "high"] || theme.warning, variantMetaAlpha())}>
                        {local.model.variant.current() || "High"}
                      </text>
                    </box>
                  </Show>
                </Show>
                  ...
                  <box flexDirection="row" gap={2} alignItems="center">
                    <Show when={usage()}>
                      {(item) => (
                        <text fg={theme.textMuted} wrapMode="none">
                          {[item().cost, item().context].filter(Boolean).join(" · ")}
                        </text>
                      )}
                    </Show>
                    <Show when={!props.homeDock}>
                      <box
                        ...
                        height={1}
                        onMouseUp={async () => {
                          const { DialogExtension } = await import("@/cli/cmd/tui/component/dialog-extension")
                          dialog.replace(() => <DialogExtension />)
                          dialog.setSize("medium")
                        }}
                      >
                        <text fg={theme.primary} attributes={TextAttributes.BOLD} wrapMode="none">@</text>
                        <text fg={theme.textMuted} wrapMode="none">Extension</text>
                      </box>
                      <MemoryButton />
                    </Show>
                  </box>
