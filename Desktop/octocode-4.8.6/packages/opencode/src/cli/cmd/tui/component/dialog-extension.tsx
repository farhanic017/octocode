...
export function DialogExtension() {
  ...
  if (installed) {
    if (!sdk.url) {
      toast.show({ message: "No live server active", variant: "warning", duration: 3000 })
      dialog.clear()
      return <box />
    }
    try {
      ...
      setTimeout(() => {
        ...
        toast.show({ message: "Extension browser opened with live server", variant: "info", duration: 3000 })
      }, 1000)
    } catch {
      toast.show({ message: "Extension browser started", variant: "info", duration: 3000 })
    }
    dialog.clear()
    return <box />
  }
      ...
      onSelect={async (option) => {
        ...
        try {
          ...
          if (sdk.url) {
            ...
            proc.stdin.write(JSON.stringify({ action: "navigate", url: sdk.url, id: 1 }) + "\n")
            toast.show({ message: "Extension installed and browser started", variant: "success", duration: 3000 })
          } else {
            toast.show({ message: "Extension installed. No live server active yet.", variant: "info", duration: 3000 })
          }
          dialog.clear()
        } catch (e: any) {
