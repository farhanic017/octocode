...
import type { Hooks } from "@octo-ai/plugin"
...
export const ProvidersLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "octocode auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in octocode.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via octocode.json options (profile, region, endpoint) or\n" +
...
export const ProvidersWhoamiCommand = cmd({
  ...
  async handler(_args) {
    ...
    if (!info) {
      prompts.log.error("Not logged in. Run `octo auth login` to log in.")
      return
    }
