...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@octo-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run octocode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running octocode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      const headers = (() => {
        ...
        const username = process.env.MIMOCODE_SERVER_USERNAME ?? "octocode"
