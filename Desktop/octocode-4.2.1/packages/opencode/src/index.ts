...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    ...
    try {
      const { migrateIfNeeded } = await import("./util/migration")
      const homeDir = process.env.HOME || process.env.USERPROFILE || ""
      const result = await migrateIfNeeded(homeDir)
      if (result.config || result.directory || result.database || result.projectId) {
        Log.Default.info("migration completed", result)
      }
    } catch (e) {
      Log.Default.warn("migration failed", { error: e })
    }
    ...
    try {
      const { verifyGuardrailIntegrity } = await import("./guardrail/integrity")
      if (!verifyGuardrailIntegrity()) {
        Log.Default.error("guardrail-integrity-check-failed")
        process.stderr.write("Critical error: Guardrail system integrity check failed.\n")
        process.stderr.write("OctoCode cannot run without a valid guardrail system.\n")
        process.exit(1)
      }
      Log.Default.info("guardrail-integrity-check-passed")
    } catch (e) {
      Log.Default.error("guardrail-integrity-error", { error: String(e) })
      process.stderr.write("Critical error: Guardrail system integrity check failed.\n")
      process.exit(1)
    }
