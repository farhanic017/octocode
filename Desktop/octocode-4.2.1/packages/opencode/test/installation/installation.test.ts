...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when @octo-ai/cli is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "@octo-ai/cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when @octo-ai/cli is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "@octo-ai/cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when @octo-ai/cli is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "@octo-ai/cli@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has @octo-ai/cli", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
  ...
  describe("latest", () => {
    test("reads version from npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          expect(req.url).toContain(encodeURIComponent("@octo-ai/cli"))
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package", async () => {
      ...
      expect(capturedArgs).toContain("@octo-ai/cli@2.0.0")
    })
