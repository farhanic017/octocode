...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "octocode-core-test-"))
  return {
    path: dir,
...
describe("cross-spawn spawner", () => {
  ...
  describe("combined output (all)", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        const handle = yield* ChildProcess.make("node", ["-e", "process.stdout.write('hello from stdout')"])
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["OCTOCODE_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { OCTOCODE_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("OCTOCODE_TEST_SHELL=ok")
      }),
    )
