...
import { ConfigReference } from "@octocode-ai/core/config/reference"
...
import { ProjectReference } from "@octocode-ai/core/project-reference"
import { Repository } from "@octocode-ai/core/repository"
import { RepositoryCache } from "@octocode-ai/core/repository-cache"
...
describe("ProjectReference", () => {
  it.live("uses the broad experimental flag unless references are explicitly configured", () =>
    withEnv(
      { OCTOCODE_EXPERIMENTAL: "true", OCTOCODE_EXPERIMENTAL_REFERENCES: undefined },
      Effect.sync(() => {
        expect(Flag.OCTOCODE_EXPERIMENTAL_REFERENCES).toBe(true)
      }),
    ).pipe(
      Effect.flatMap(() =>
        withEnv(
          { OCTOCODE_EXPERIMENTAL: "true", OCTOCODE_EXPERIMENTAL_REFERENCES: "false" },
          Effect.sync(() => {
            expect(Flag.OCTOCODE_EXPERIMENTAL_REFERENCES).toBe(false)
          }),
        ),
      ),
    ),
...
function withReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ OCTOCODE_EXPERIMENTAL_REFERENCES: "true" }, body)
}

function withoutReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ OCTOCODE_EXPERIMENTAL: undefined, OCTOCODE_EXPERIMENTAL_REFERENCES: undefined }, body)
}
