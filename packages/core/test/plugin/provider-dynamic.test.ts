...
import { AISDK } from "@octocode-ai/core/aisdk"
...
import { DynamicProviderPlugin } from "@octocode-ai/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "octocode-provider-dynamic-"))
