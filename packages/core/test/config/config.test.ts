...
import { ConfigProvider } from "@octocode-ai/core/config/provider"
import { ConfigMigrateV1 } from "@octocode-ai/core/v1/config/migrate"
...
import { Policy } from "@octocode-ai/core/policy"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "octocode.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "octocode.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "octocode.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "octocode.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "octocode.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "octocode.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .octocode configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".octocode"), { recursive: true })
            await fs.mkdir(path.join(directory, ".octocode"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "octocode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "octocode.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "octocode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "octocode.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".octocode", "octocode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".octocode", "octocode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".octocode")),
              AbsolutePath.make(path.join(directory, ".octocode")),
            ])
