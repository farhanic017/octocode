...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@octocode/acme")).toBe("@octocode/acme")
    expect(Npm.sanitize("@octocode/acme@1.0.0")).toBe("@octocode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/octocode/acme.git"
    const expected = win ? "acme@git+https_//github.com/octocode/acme.git" : spec
