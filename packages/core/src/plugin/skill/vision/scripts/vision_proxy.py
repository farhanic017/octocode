...
Tries free backends first, then falls back to others in parallel.
  Priority: MiMo → Gemini → Azure → Groq → HuggingFace → Mistral in parallel
  Total timeout: 25s | Per-backend timeout: 12s

Custom model (auto-routes to best provider):
  --model "xiaomi/mimo-v2.5"           → OpenRouter (MiMo)
  --model "xiaomi/mimo-v2-omni"        → OpenRouter (MiMo)
...
Supported provider keys (set via env vars):
  GEMINI_API_KEY | CLOUDFLARE_API_KEY | AZUREAI_API_KEY | AZUREAI_ENDPOINT | ANTHROPIC_API_KEY | MISTRAL_API_KEY | GROQ_API_KEY | HF_TOKEN | FIREWORKS_API_KEY | ZAI_API_KEY | OPENROUTER_API_KEY

Usage:
  python vision_proxy.py <image_or_video_path> [prompt text...] [--model NAME]

First run? Set at least one API key:
...
def load_config():
    ...
    if not present:
        raise RuntimeError(
            ...
            "  Set at least one environment variable (any one is enough):\n"
            "    $env:GEMINI_API_KEY='...'            (aistudio.google.com/apikey)\n"
            "    $env:OPENROUTER_API_KEY='sk-or-...'  (openrouter.ai/keys)\n"
...
def _has_key(name):
    ...
    if "MiMo" in name or "mimo" in name:
        return bool(CFG.get("XIAOMI_API_KEY")) or bool(CFG.get("OPENROUTER_API_KEY"))
    if "Xiaomi" in name or "xiaomi" in name:
        return bool(CFG.get("XIAOMI_API_KEY")) or bool(CFG.get("OPENROUTER_API_KEY"))
...
XIAOMI_ENDPOINT = _api_http_base(os.environ.get("XIAOMI_BASE_URL"), "https://token-plan-cn.xiaomimimo.com/v1")
...
# ── Xiaomi MiMo caller (OpenAI-compatible) ────────────────────────


def _xiaomi_headers():
    return {
        "Authorization": f"Bearer {CFG['XIAOMI_API_KEY']}",
        "Content-Type": "application/json",
    }


def call_xiaomi(b64data, mime, prompt, model="mimo-v2.5"):
    payload = {
        "model": model,
        "messages": [{"role": "user", "content": [
            {"type": "text", "text": prompt},
    ...
    req = urllib.request.Request(
        f"{XIAOMI_ENDPOINT}/chat/completions",
        data=json.dumps(payload).encode(),
        headers=_xiaomi_headers(),
    )
    resp = urllib.request.urlopen(req, timeout=30)
    return json.loads(resp.read())["choices"][0]["message"]["content"]


def call_xiaomi_multi(frames, prompt, model="mimo-v2.5"):
    payload = {
        "model": model,
        "messages": [{"role": "user", "content": build_multimodal_content(frames, prompt)}],
        "max_tokens": 2048,
...
def get_providers_for_model(model):
    ...
    if "/" in model:
        ...
        if prefix == "xiaomi":
            return _filter_providers([("xiaomi", stripped)])
    ...
    if "mimo" in ml:
        if CFG and CFG.get("XIAOMI_API_KEY"):
            return [("xiaomi", model)]
        return _filter_providers([("openrouter", f"xiaomi/{model}" if "/" not in model else model)])
    return []
...
def _build_strategies(kind, *args, prompt=""):
    use_xiaomi_direct = CFG and CFG.get("XIAOMI_API_KEY")
    if kind == "vid":
        frames = args[0]
        s = [
            # Xiaomi MiMo — tried first (free, multimodal)
            ("\u2606 MiMo V2.5", lambda: (call_xiaomi_multi if use_xiaomi_direct else call_openrouter_multi)(frames, prompt, "mimo-v2.5" if use_xiaomi_direct else "xiaomi/mimo-v2.5")),
            ("\u2606 MiMo V2 Omni", lambda: (call_xiaomi_multi if use_xiaomi_direct else call_openrouter_multi)(frames, prompt, "mimo-v2-omni" if use_xiaomi_direct else "xiaomi/mimo-v2-omni")),
            # Google Gemini — tried next (fastest, most reliable)
        ...
        s = [
            # Xiaomi MiMo — tried first (free, multimodal)
            ("\u2606 MiMo V2.5", lambda: (call_xiaomi if use_xiaomi_direct else call_openrouter)(img_b64, mime, prompt, "mimo-v2.5" if use_xiaomi_direct else "xiaomi/mimo-v2.5")),
            ("\u2606 MiMo V2 Omni", lambda: (call_xiaomi if use_xiaomi_direct else call_openrouter)(img_b64, mime, prompt, "mimo-v2-omni" if use_xiaomi_direct else "xiaomi/mimo-v2-omni")),
            # Google Gemini — tried next (fastest, most reliable)
...
def _insert_model_strategies(strategies, model, kind, *args, prompt=""):
    dispatch = {
        ...
        "xiaomi": (call_xiaomi, call_xiaomi_multi),
    }
    is_vid = kind == "vid"
...
def analyze(file_path, prompt="", model=None):
    ...
    if not strategies:
        ...
        raise RuntimeError("No backends available — configure at least one API key")
...
def _list_models():
    ...
    models = [
        # Xiaomi MiMo
        ("\u2606 MiMo V2.5",                     "OPENROUTER_API_KEY"),
        ("\u2606 MiMo V2 Omni",                   "OPENROUTER_API_KEY"),
        # Azure AI Foundry
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Set at least one API key: $env:GEMINI_API_KEY='...'",
    )
