...
export class Info extends Schema.Class<Info>("Config.Info")({
  ...
  obsidian: ConfigObsidian.Info.pipe(Schema.optional).annotate({
    description: "Obsidian vault integration for cross-session project context",
  }),
...
export class Service extends Context.Service<Service, Interface>()("@octocode/v2/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["config.json", "octocode.json", "octocode.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".octocode", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".octocode")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".octocode").toReversed()
    ...
    // global config, project files, then `.octocode` files.
