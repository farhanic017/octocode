...
import { ToolFailure, toolText } from "@octocode-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@octocode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.OCTOCODE_WEBSEARCH_PROVIDER === "exa" || process.env.OCTOCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.OCTOCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("OCTOCODE_EXPERIMENTAL") || truthy("OCTOCODE_ENABLE_EXA") || truthy("OCTOCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("OCTOCODE_ENABLE_PARALLEL") || truthy("OCTOCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `octocode/${InstallationVersion}`,
