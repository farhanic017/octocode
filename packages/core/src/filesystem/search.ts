...
export class Service extends Context.Service<Service, Interface>()("@octocode/Search") {}
...
export const layer: Layer.Layer<Service, never, FSUtil.Service | Ripgrep.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const acquire = Effect.fn("Search.acquire")(function* (cwd: string) {
      // The octocode test runtime owns an isolated XDG tree that Windows must
      // remove before process exit, so use ripgrep instead of native FFF there.
      if (process.env.OCTOCODE_TEST_HOME) return undefined
      ...
      return yield* Effect.gen(function* () {
        ...
        const made = yield* fffSync("create picker", () =>
          Fff.create({
            ...
            // home directory, if the user specifically opened octocode at the
    ...
    // in 99% of use cases user that is opened octocode at certain directory will
