...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve octocode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "octo")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "octo")).toBe("openai")
    expect(statProvider("big-pickle", "", "octo")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "octo"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep octocode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "octo"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "octo"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep octocode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "octo"), country: "US" })).toMatchObject([
