...
import {
  ...
  type StatsModelData,
  type UsageRange,
} from "@octocode-ai/stats-core/domain/home"
import { runtime } from "@octocode-ai/stats-core/runtime"
...
const statsModelFallbackUrl = "https://stats.octocode.ai"
...
export default function StatsModel() {
  ...
  const modelDescription = createMemo(() =>
    stats()
      ? `${modelName()} usage, rank, token mix, cost, geo breakdown, and peer stats across OctoCode.`
      : `${modelName()} model facts, limits, and OctoCode usage availability.`,
  )
      ...
      <div data-slot="model-hero-grid">
        <div data-slot="model-hero-copy">
          ...
          <Show
            when={props.data}
            fallback={
              <p>Model facts from the shared model index. OctoCode usage appears once this model has activity.</p>
            }
          >
            {(data) => (
              <p>
                Ranked #{data().rank} across recent OctoCode token usage with {formatPercent(data().tokenShare)} of
                observed volume.
              </p>
            )}
          </Show>
...
function ModelCatalogCallout(props: { catalog: ModelCatalogEntry | null }) {
  return (
    <div data-component="model-rank-panel">
      ...
      <p>No OctoCode usage in the current stats window.</p>
    </div>
  )
}
...
function ModelOverview(props: { data: StatsModelData | null }) {
  return (
    <section data-section="model-panel">
      ...
      <Show
        when={props.data}
        fallback={<ModelEmptyState title="No usage summary" description="This model has no OctoCode usage rows yet." />}
      >
        {(data) => (
