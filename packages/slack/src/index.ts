...
import { createOctocode, type ToolPart } from "@octocode-ai/sdk"
...
console.log("🚀 Starting octocode server...")
const octocode = await createOctocode({
  port: 0,
})
console.log("✅ OctoCode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string; lastActivity: number }>()
...
void (async () => {
  const events = await octocode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new octocode session...")
    const { client, server } = octocode
    ...
    console.log("✅ Created octocode session:", createResult.data.id)

    session = { client, server, sessionId: createResult.data.id, channel, thread, lastActivity: Date.now() }
  ...
  session.lastActivity = Date.now()
  console.log("📝 Sending to octocode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 OctoCode response:", JSON.stringify(result, null, 2))
