...
const reg = process.env.REGISTRY ?? "ghcr.io/farhanic017"
...
// Handle "bun@latest" by using "latest" as the version
const bunVersion = bun === "latest" ? "latest" : bun
...
const setup = async () => {
  ...
  if (list.includes("octo")) {
    await $`docker buildx use octocode`
    return
  }
  await $`docker buildx create --name octocode --use`
}
...
for (const name of images) {
  ...
  if (name === "bun-node") {
    if (push) {
      console.log(
        `docker buildx build --platform ${platform} -f ${file} -t ${image} --build-arg REGISTRY=${reg} --build-arg BUN_VERSION=${bunVersion} --push .`,
      )
      await $`docker buildx build --platform ${platform} -f ${file} -t ${image} --build-arg REGISTRY=${reg} --build-arg BUN_VERSION=${bunVersion} --push .`
    }
    if (!push) {
      console.log(`docker build -f ${file} -t ${image} --build-arg REGISTRY=${reg} --build-arg BUN_VERSION=${bunVersion} .`)
      await $`docker build -f ${file} -t ${image} --build-arg REGISTRY=${reg} --build-arg BUN_VERSION=${bunVersion} .`
    }
  }
