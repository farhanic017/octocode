...
import { LocationServiceMap } from "@octocode-ai/core/location-layer"
...
>()("@octocode/HttpApiLocation") {}

function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  const query = new URL(request.url, "http://localhost").searchParams
  const workspaceID = query.get("location[workspace]") || request.headers["x-octocode-workspace"]
  return {
    directory: AbsolutePath.make(
      query.get("location[directory]") || request.headers["x-octocode-directory"] || process.cwd(),
    ),
