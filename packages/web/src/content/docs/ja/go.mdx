...
OctoCode Goは低価格のサブスクリプションで、**初月は5ドル**、その後は**月額10ドル**で、人気のオープンなコーディングモデルに安定してアクセスできます。

GoはOctoCodeの他のプロバイダーと同様に機能します。OctoCode GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、OctoCodeを使用するために必須ではありません。
...
:::tip
OctoCodeでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
OctoCode Goを使用すると、これらのモデルに**初月は5ドル**、その後は**月額10ドル**でアクセスできます。

---

## 仕組み

OctoCode Goは、OctoCodeの他のプロバイダーと同様に機能します。

1. **<a href={console}>OctoCode Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`OctoCode Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
OctoCode Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
OctoCode設定の[model id](/docs/config/#models)は、`octocode-go/<model-id>`という形式を使用します。たとえば、Kimi K2.6の場合は、設定で`octocode-go/kimi-k2.6`を使用します。
...
4. OctoCodeで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
