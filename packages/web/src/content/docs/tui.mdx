---
title: TUI
description: Using the OctoCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OctoCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OctoCode starts the TUI for the current directory.
...
When using the OctoCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OctoCode. Allows you to select from available providers and add their API keys.
...
Exit OctoCode. _Aliases_: `/quit`, `/q`
...
This is separate from `octocode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long OctoCode waits after the leader key. Defaults to `2000`.
...
Use `OCTOCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `octocode.default`.
