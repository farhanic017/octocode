---
title: Plugins
description: Escreva seus próprios plugins para estender o octocode.
---

Plugins permitem que você estenda o octocode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do octocode.
...
- `.octocode/plugins/` - Plugins em nível de projeto
- `~/.config/octocode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/octocode/node_modules/`.
...
1. Configuração global (`~/.config/octocode/octocode.json`)
2. Configuração do projeto (`octocode.json`)
3. Diretório de plugins global (`~/.config/octocode/plugins/`)
4. Diretório de plugins do projeto (`.octocode/plugins/`)
...
O octocode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do octocode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o octocode.
...
:::note
Se você estiver usando o aplicativo desktop octocode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o octocode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao octocode:
...
O helper `tool` cria uma ferramenta personalizada que o octocode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o octocode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://octocode.ai/docs/sdk) para detalhes.
