---
title: Servidor
description: Interaja com o servidor octocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `octocode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente octocode pode usar.
...
```bash
octocode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `octocode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
octocode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OCTOCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `octo`, ou defina `OCTOCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `octocode serve` quanto ao `octocode web`.

```bash
OCTOCODE_SERVER_PASSWORD=your-password octocode serve
```
...
Quando você executa `octo`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor octocode para interagir com o octocode programaticamente.
:::

Essa arquitetura permite que o octocode suporte múltiplos clientes e permite que você interaja com o octocode programaticamente.

Você pode executar `octocode serve` para iniciar um servidor autônomo. Se você tiver o TUI do octocode em execução, `octocode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do octocode [IDE](/docs/ide).
...
O servidor octocode expõe as seguintes APIs.
