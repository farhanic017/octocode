...
Com o octocode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o octocode usa nosso próprio tema `octo`.
...
O octocode vem com vários temas embutidos.
...
- Querem que o octocode corresponda à aparência do seu terminal
...
O octocode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/octocode/themes/*.json` ou `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.octocode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.octocode/themes/*.json`
