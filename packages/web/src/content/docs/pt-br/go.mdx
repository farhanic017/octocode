...
O OctoCode Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no OctoCode. Você assina o OctoCode Go e
...
O OctoCode Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O OctoCode Go funciona como qualquer outro provedor no OctoCode.

1. Você entra no **<a href={console}>OctoCode Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `OctoCode Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o OctoCode Go.
:::
...
O OctoCode Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do OctoCode
usa o formato `octocode-go/<model-id>`. Por exemplo, para o Kimi K2.6, você usaria
`octocode-go/kimi-k2.6` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o OctoCode também.
