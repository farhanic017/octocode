...
As habilidades do agente permitem que o octocode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O octocode pesquisa nesses locais:

- Configuração do projeto: `.octocode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/octocode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o octocode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.octocode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/octocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.octocode/skills/git-release/SKILL.md` assim:
...
O octocode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `octocode.json`:
...
**Para agentes embutidos** (em `octocode.json`):
