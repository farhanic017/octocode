---
title: Provedores
description: Usando qualquer provedor LLM no octocode.
---

import config from "../../../../config.mjs"
export const console = config.console

octocode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do octocode.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/octocode/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do octocode.
...
OctoCode Zen é uma lista de modelos fornecidos pela equipe do octocode que foram testados e verificados para funcionar bem com o octocode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o OctoCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `OctoCode Zen` e acesse [octocode.ai/zen](https://octocode.ai/zen).
...
Funciona como qualquer outro provedor no octocode e é completamente opcional.

---

## OctoCode Go

OctoCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do octocode que foram
testados e verificados para funcionar bem com o octocode.

1. Execute o comando `/connect` no TUI, selecione `OctoCode Go` e acesse [octocode.ai/zen](https://octocode.ai/zen).
...
Para usar o Amazon Bedrock com o octocode:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o octocode:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `octocode.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no octocode não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o octocode para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o octocode funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do octocode.
...
armazenar o token no armazenamento de autenticação do octocode.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
octocode usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o octocode
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `octocode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [octocode-gitlab-auth](https://www.npmjs.com/package/octocode-gitlab-auth).

##### Configuração

Personalize através do `octocode.json`:
...
Para usar sua assinatura do GitHub Copilot com o octocode:
...
Para usar o Google Vertex AI com o octocode:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o octocode.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do octocode, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [octocode-helicone-session](https://github.com/H2Shami/octocode-helicone-session) para registrar automaticamente cada conversa do octocode como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do octocode listada como uma sessão separada.
...
Você pode configurar o octocode para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o octocode para usar modelos locais através do LM Studio.
...
Você pode configurar o octocode para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o octocode. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/octocode) para detalhes.
:::
...
Para usar o Ollama Cloud com o octocode:
...
3. Copie a chave da API para uso no octocode.
...
6. **Importante**: Antes de usar modelos em nuvem no octocode, você deve puxar as informações do modelo localmente:
...
OctoCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do octocode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>OctoCode Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **OctoCode Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do octocode.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do octocode.
...
5. Você também pode personalizá-los através da sua configuração do octocode. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o octocode:
...
Você também pode personalizar modelos através da sua configuração do octocode. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o octocode. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no octocode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `octocode.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o octocode entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `octocode auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do octocode e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do octocode.
