...
You can add external tools to OctoCode using the _Model Context Protocol_, or MCP. OctoCode supports both local and remote servers.
...
You can define MCP servers in your [OctoCode Config](https://octocode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/octocode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OctoCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OctoCode will:
...
If the server requires authentication, OctoCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `octocode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OctoCode will store the tokens securely in `~/.local/share/octocode/mcp-auth.json`.
...
Your MCPs are available as tools in OctoCode, alongside built-in tools. So you can manage them through the OctoCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OctoCode to your Sentry account.
