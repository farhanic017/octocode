...
Avec OctoCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OctoCode utilise notre propre thème `octo`.
...
OctoCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OctoCode corresponde à l'apparence de leur terminal
...
OctoCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/octocode/themes/*.json` ou `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.octocode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.octocode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/octocode/themes
vim ~/.config/octocode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .octocode/themes
vim .octocode/themes/my-theme.json
```
...
{
  "$schema": "https://octocode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
