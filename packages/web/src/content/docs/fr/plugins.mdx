---
title: Plugins
description: Écrivez vos propres plugins pour étendre OctoCode.
---

Les plugins vous permettent d'étendre OctoCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OctoCode.
...
- `.octocode/plugins/` - Plugins au niveau du projet
- `~/.config/octocode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/octocode/node_modules/`.
...
1. Configuration globale (`~/.config/octocode/octocode.json`)
2. Configuration du projet (`octocode.json`)
3. Répertoire global des plugins (`~/.config/octocode/plugins/`)
4. Répertoire des plugins du projet (`.octocode/plugins/`)
...
OctoCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK octocode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre octocode.
...
:::note
Si vous utilisez l'application de bureau OctoCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez octocode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à octocode :
...
L'assistant `tool` crée un outil personnalisé que octocode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour octocode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://octocode.ai/docs/sdk) pour plus de détails.
