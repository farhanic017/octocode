---
title: SDK
description: Client JS de type sécurisé pour le serveur octocode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK octocode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler octocode par programme.
...
Créez une instance de octocode :

```javascript
import { createOctocode } from "@octocode-ai/sdk"

const { client } = await createOctocode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `octocode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${octocode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de octocode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@octocode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*octocode" },
})
