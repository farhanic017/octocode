---
title: Serveur
description: Interagissez avec le serveur octocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `octocode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client octocode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `octocode.local` |
...
Définissez `OCTOCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `octo`, ou définissez `OCTOCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `octocode serve` et à `octocode web`.
...
Lorsque vous exécutez `octo`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur octocode pour interagir avec octocode par programmation.
:::

Cette architecture permet à octocode de prendre en charge plusieurs clients et vous permet d'interagir avec octocode par programme.

Vous pouvez exécuter `octocode serve` pour démarrer un serveur autonome. Si vous avez le
octocode TUI en cours d'exécution, `octocode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OctoCode [IDE](/docs/ide).
...
Le serveur octocode expose les API suivantes.
