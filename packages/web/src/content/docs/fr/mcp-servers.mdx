...
Vous pouvez ajouter des outils externes à OctoCode à l'aide du _Model Context Protocol_ ou MCP. OctoCode prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [OctoCode Config](https://octocode.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/octocode`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
OctoCode gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, OctoCode :
...
Si le serveur nécessite une authentification, OctoCode vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `octocode mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, OctoCode stockera les jetons en toute sécurité dans `~/.local/share/octocode/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans OctoCode, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration OctoCode comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter OctoCode à votre compte Sentry.
