...
With OctoCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OctoCode uses our own `octo` theme.
...
OctoCode comes with several built-in themes.
...
- Want OctoCode to match their terminal's appearance
...
OctoCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/octocode/themes/*.json` or `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.octocode/themes/*.json`
4. **Current working directory** - Defined in `./.octocode/themes/*.json`
