...
Создайте `.octocode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию octocode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем octocode [config](/docs/config):
...
- Глобальный: `~/.config/octocode/commands/`
- Для каждого проекта: `.octocode/commands/`
...
octocode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
