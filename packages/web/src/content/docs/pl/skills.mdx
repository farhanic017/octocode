...
Umiejętności agenta pozwalają octocode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
octocode przeszukuje te lokalizacje:

- Project config: `.octocode/skills/<name>/SKILL.md`
- Global config: `~/.config/octocode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu octocode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.octocode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/octocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.octocode/skills/git-release/SKILL.md` w ten sposób:
...
octocode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `octocode.json`:
...
**Dla agentów wbudowanych** (w `octocode.json`):
