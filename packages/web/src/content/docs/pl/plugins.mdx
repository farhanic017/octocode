---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć octocode.
---

Wtyczki umożliwiające wyłączenie octocode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania octocode.
...
- `.octocode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/octocode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/octocode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/octocode/octocode.json`)
2. Konfiguracja projektu (`octocode.json`)
3. Globalny katalog wtyczek (`~/.config/octocode/plugins/`)
4. Katalog wtyczek projektu (`.octocode/plugins/`)
...
octocode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z octocode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do octocode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej octocode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj octocode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do octocode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod octocode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://octocode.ai/docs/sdk).
