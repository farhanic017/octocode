...
Dzięki octocode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie octocode używa naszego własnego motywu `octo`.
...
octocode ma kilka wbudowanych motywów.
...
- Chcą, aby OctoCode pasował do wyglądu ich terminala
...
octocode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/octocode/themes/*.json` lub `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.octocode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.octocode/themes/*.json`
