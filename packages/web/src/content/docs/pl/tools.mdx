...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. OctoCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w OctoCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OCTOCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `OCTOCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OctoCode lub gdy zmienna środowiskowa `OCTOCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu OctoCode:
