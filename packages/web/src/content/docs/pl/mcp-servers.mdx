...
Możesz dodać dodatkowe informacje do octocode za pomocą _Model Context Protocol_, w skrócie MCP. octocode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [octocode Config](https://octocode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/octocode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
octocode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, octocode:
...
Jeśli serwer wymaga uwierzytelnienia, octocode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `octocode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji octocode bezpieczne przechowa tokeny w `~/.local/share/octocode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w octocode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie octocode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć octocode z kontem Sentry.
