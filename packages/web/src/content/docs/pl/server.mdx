---
title: Serwer
description: Interakcja z serwerem octocode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `octocode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient octocode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `octocode.local` |
...
Ustaw `OCTOCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `octo` lub ustaw `OCTOCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `octocode serve`, jak i `octocode web`.
...
Kiedy uruchomisz `octo`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera octocode do programowej interakcji z octocode.
:::

Ta architektura umożliwia obsługę wielu klientów przez octocode i programową interakcję z kodem otwartym.

Możesz uruchomić `octocode serve`, aby uruchomić samodzielny serwer. Jeśli masz
octocode TUI działa, `octocode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki octocode [IDE](/docs/ide).
...
Serwer octocode udostępnia następujące interfejsy API.
