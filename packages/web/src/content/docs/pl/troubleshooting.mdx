...
Aby debugować problemy z octocode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\octocode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `octocode --log-level DEBUG`.

---

## Przechowywanie danych

octocode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/octocode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\octocode`
...
octocode Desktop uruchamia w tle lokalny serwer octocode (sidecar `octocode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `OctoCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/octocode/octocode.jsonc` (lub `~/.config/octocode/octocode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/octocode/octocode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\octocode\octocode.jsonc`
...
octocode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/octocode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\octocode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby octocode mógł ją odbudować.

1. Całkowicie zamknij octocode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/octocode`
- **Linux**: usuń `~/.cache/octocode` (lub uruchom `rm -rf ~/.cache/octocode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\octocode`

3. Uruchom ponownie octocode Desktop.

---

### Napraw problemy z połączeniem z serwerem

octocode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `octocode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OCTOCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OCTOCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows octocode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami octocode.

---

### Brak powiadomień

octocode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla octocode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie octocode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji octocode Desktop):

- `octocode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `octocode.global.dat` i `octocode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z octocode:
...
3. Upewnij się, że masz najnowszą wersję z `octocode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `octocode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\octocode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. octocode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\octocode`

2. Uruchom ponownie kod octocode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to octocode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
octocode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
