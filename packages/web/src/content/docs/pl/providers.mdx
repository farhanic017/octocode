---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w octocode.
---

import config from "../../../../config.mjs"
export const console = config.console

octocode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji octocode.
...
w `~/.local/share/octocode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim octocode
config.
...
OctoCode Zen to lista modeli dostarczonych przez zespół octocode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z octocode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OctoCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OctoCode Zen` i przejdź do [octocode.ai/auth](https://octocode.ai/zen).
...
Działa jak każdy inny dostawca w octocode i jest całkowicie opcjonalny w użyciu.

---

## OctoCode Go

OctoCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół octocode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z octocode.

1. Uruchom polecenie `/connect` w TUI, wybierz `OctoCode Go` i przejdź do [octocode.ai/auth](https://octocode.ai/zen).
...
Aby używać Amazon Bedrock z octocode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania octocode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `octocode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w octocode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować octocode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod octocode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji octocode.
...
octocode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować octocode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`octocode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [octocode-gitlab-auth](https://www.npmjs.com/package/octocode-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem octocode:
...
Aby używać Google Vertex AI z octocode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania octocode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą octocode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [octocode-helicone-session](https://github.com/H2Shami/octocode-helicone-session), aby automatycznie rejestrować każdą konwersację octocode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja octocode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować octocode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować octocode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować octocode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem octocode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/octocode).
:::
...
Aby korzystać z Ollama Cloud z octocode:
...
3. Skopiuj klucz API do użycia w octocode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w octocode musisz pobrać informacje o modelu lokalnie:
...
OctoCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół octocode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OctoCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OctoCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API octocode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji octocode.
...
5. Możesz je także dostosować za pomocą konfiguracji octocode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z octocode:
...
Możesz także dostosować modele za pomocą konfiguracji octocode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z octocode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `octocode.json` w katalogu projektu:
...
Pola `limit` pozwalają octocode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `octocode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację octocode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji octocode.
