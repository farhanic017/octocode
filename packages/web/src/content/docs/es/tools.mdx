...
Las herramientas permiten que LLM realice acciones en su código base. OctoCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en OctoCode.
...
:::note
Esta herramienta solo está disponible cuando `OCTOCODE_EXPERIMENTAL_LSP_TOOL=true` (o `OCTOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OctoCode o cuando la variable de entorno `OCTOCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OctoCode:

```bash
OCTOCODE_ENABLE_EXA=1 octocode
```
