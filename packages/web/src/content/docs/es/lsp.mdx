...
description: OctoCode se integra con sus servidores LSP.
---

OctoCode puede integrarse con servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para el agente.

---

## Incorporados

OctoCode viene con varios servidores LSP integrados para idiomas populares:
...
:::note
Puede deshabilitar las descargas automáticas del servidor LSP configurando la variable de entorno `OCTOCODE_DISABLE_LSP_DOWNLOAD` en `true`.
:::
...
Cuando LSP está habilitado y octocode abre un archivo:
...
Puede habilitar y personalizar los servidores LSP a través de la sección `lsp` en su configuración octocode.
