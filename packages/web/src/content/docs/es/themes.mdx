...
Con OctoCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, OctoCode usa nuestro propio tema `octo`.
...
OctoCode viene con varios temas integrados.
...
- Quiere que OctoCode coincida con la apariencia de su terminal
...
OctoCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/octocode/themes/*.json` o `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.octocode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.octocode/themes/*.json`
