---
title: Complementos
description: Escriba sus propios complementos para extender OctoCode.
---

Los complementos le permiten extender OctoCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OctoCode.
...
- `.octocode/plugins/` - Complementos a nivel de proyecto
- `~/.config/octocode/plugins/` - Complementos globales
...
```json title="octocode.json"
{
  ...
  "plugin": ["octocode-helicone-session", "octocode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/octocode/node_modules/`.
...
1. Configuración global (`~/.config/octocode/octocode.json`)
2. Configuración del proyecto (`octocode.json`)
3. Directorio global de complementos (`~/.config/octocode/plugins/`)
4. Directorio de complementos del proyecto (`.octocode/plugins/`)
...
```json title=".octocode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
OctoCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".octocode/plugins/my-plugin.ts"
...
```js title=".octocode/plugins/example.js"
...
- `client`: Un cliente SDK octocode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar octocode.
...
```js title=".octocode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "octo"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio OctoCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que octocode lea archivos `.env`:

```javascript title=".octocode/plugins/env-protection.js"
...
```javascript title=".octocode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a octocode:

```ts title=".octocode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@octocode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que octocode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para octocode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://octocode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".octocode/plugins/compaction.ts"
import type { Plugin } from "@octocode-ai/plugin"
...
```ts title=".octocode/plugins/custom-compaction.ts"
import type { Plugin } from "@octocode-ai/plugin"
