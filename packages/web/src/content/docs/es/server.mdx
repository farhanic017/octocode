---
title: Servidor
description: Interactuar con el servidor octocode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `octocode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente octocode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `octocode.local` |
...
Configure `OCTOCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `octo`, o configure `OCTOCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `octocode serve` como a `octocode web`.
...
Cuando ejecuta `octo`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor octocode para interactuar con octocode mediante programación.
:::

Esta arquitectura permite que octocode admita múltiples clientes y le permite interactuar con octocode mediante programación.

Puede ejecutar `octocode serve` para iniciar un servidor independiente. Si tienes el
octocode TUI ejecutándose, `octocode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OctoCode [IDE](/docs/ide).
...
El servidor octocode expone las siguientes API.
