...
description: Cree herramientas que LLM pueda llamar en octocode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de octocode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.octocode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/octocode/tools/`.
...
```ts title=".octocode/tools/database.ts" {1}
...
```ts title=".octocode/tools/math.ts"
import { tool } from "@octocode-ai/plugin"
...
```ts title=".octocode/tools/bash.ts"
import { tool } from "@octocode-ai/plugin"
...
```ts title=".octocode/tools/project.ts" {8}
import { tool } from "@octocode-ai/plugin"
...
```python title=".octocode/tools/add.py"
import sys
...
```ts title=".octocode/tools/python-add.ts" {10}
import { tool } from "@octocode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".octocode/tools/add.py")
