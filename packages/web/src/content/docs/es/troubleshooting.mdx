...
Para depurar problemas con OctoCode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/octocode/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\octocode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `octocode --log-level DEBUG`.

---

## Almacenamiento

octocode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/octocode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\octocode`
...
OctoCode Desktop ejecuta un servidor OctoCode local (el sidecar `octocode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `OctoCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/octocode/octocode.jsonc` (o `~/.config/octocode/octocode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/octocode/octocode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\octocode\octocode.jsonc`
...
OctoCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/octocode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\octocode\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.octocode/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que OctoCode pueda reconstruirlo.

1. Salga de OctoCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/octocode`
- **Linux**: eliminar `~/.cache/octocode` (o ejecutar `rm -rf ~/.cache/octocode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\octocode`

3. Reinicie el escritorio OctoCode.

---

### Solucionar problemas de conexión del servidor

OctoCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `octocode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `OCTOCODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `OCTOCODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio OctoCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de OctoCode.

---

### Notificaciones que no se muestran

OctoCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para OctoCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio OctoCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio OctoCode):

- `octocode.settings.dat` (URL del servidor predeterminado de escritorio)
- `octocode.global.dat` y `octocode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con OctoCode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/octocode/issues**](https://github.com/anomalyco/octocode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**octocode.ai/discord**](https://octocode.ai/discord)
...
3. Asegúrese de tener la última versión con `octocode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `octocode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\octocode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. octocode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/octocode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\octocode`

2. Reinicie octocode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a octocode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
octocode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
