...
Las habilidades del agente permiten a OctoCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OctoCode busca estas ubicaciones:

- Configuración del proyecto: `.octocode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/octocode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OctoCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.octocode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/octocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.octocode/skills/git-release/SKILL.md` así:
...
OctoCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `octocode.json`:
...
**Para agentes integrados** (en `octocode.json`):
