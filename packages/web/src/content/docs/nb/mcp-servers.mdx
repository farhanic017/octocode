...
Du kan legge til eksterne verktøy i OctoCode ved å bruke _Model Context Protocol_ eller MCP. OctoCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OctoCode Config](https://octocode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="octocode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/octocode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
OctoCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OctoCode:
...
Hvis serveren krever autentisering, vil OctoCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `octocode mcp auth <server-name>`.
...
```bash
octocode mcp auth my-oauth-server
```
...
```bash
octocode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OctoCode lagre tokens sikkert i `~/.local/share/octocode/mcp-auth.json`.
...
octocode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i OctoCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom OctoCode-konfigurasjonen som alle andre verktøy.
...
```json title="octocode.json" {11, 14-18}
{
  "$schema": "https://octocode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
octocode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OctoCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OctoCode-samtale oppført som en separat økt.
