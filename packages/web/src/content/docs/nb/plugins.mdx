...
description: Skriv dine egne programtillegg for å utvide OctoCode.
---

Plugins lar deg utvide OctoCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OctoCode.
...
- `.octocode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/octocode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/octocode/octocode.json`)
2. Prosjektkonfigurasjon (`octocode.json`)
3. Global plugin-katalog (`~/.config/octocode/plugins/`)
4. Prosjektpluginkatalog (`.octocode/plugins/`)
...
OctoCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OctoCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OctoCode.
...
:::note
Hvis du bruker OctoCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OctoCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OctoCode:
...
`tool`-hjelperen lager et tilpasset verktøy som OctoCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OctoCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://octocode.ai/docs/sdk) for detaljer.
