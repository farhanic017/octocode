---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OctoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OctoCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OctoCode-konfigurasjonen.
...
i `~/.local/share/octocode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OctoCode
konfig.
...
OctoCode Zen er en liste over modeller levert av OctoCode-teamet som har vært
testet og verifisert for å fungere godt med OctoCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OctoCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OctoCode Zen` og gå til [octocode.ai/auth](https://octocode.ai/zen).
...
Det fungerer som alle andre leverandører i OctoCode og er helt valgfritt å bruke.

---

## OctoCode Go

OctoCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av OctoCode-teamet som har vært
testet og verifisert for å fungere godt med OctoCode.

1. Kjør kommandoen `/connect` i TUI, velg `OctoCode Go`, og gå til [octocode.ai/auth](https://octocode.ai/zen).
...
Slik bruker du Amazon Bedrock med OctoCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OctoCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `octocode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i octocode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere octocode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OctoCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OctoCode-konfigurasjonen.
...
for å lagre token i OctoCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OctoCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OctoCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`octocode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [octocode-gitlab-auth](https://www.npmjs.com/package/octocode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `octocode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med OctoCode:
...
Slik bruker du Google Vertex AI med OctoCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OctoCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OctoCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [octocode-helicone-session](https://github.com/H2Shami/octocode-helicone-session) plugin for automatisk å logge hver OctoCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OctoCode-samtale oppført som en separat økt.
...
Du kan konfigurere OctoCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OctoCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OctoCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OctoCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/octocode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OctoCode:
...
3. Kopier API-nøkkelen for bruk i OctoCode.
...
6. **Viktig**: Før du bruker skymodeller i OctoCode, må du hente modellinformasjonen lokalt:
...
OctoCode Zen er en liste over testede og verifiserte modeller levert av OctoCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OctoCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OctoCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OctoCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OctoCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OctoCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OctoCode:
...
Du kan også tilpasse modeller gjennom OctoCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OctoCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `octocode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OctoCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `octocode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OctoCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OctoCode-konfigurasjonen.
