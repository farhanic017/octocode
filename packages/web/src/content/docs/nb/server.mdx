---
title: Tjener
description: Samhandle med OctoCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `octocode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OctoCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `octocode.local` |
...
Sett `OCTOCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `octo` som standard, eller sett `OCTOCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `octocode serve` og `octocode web`.
...
Når du kjører `octo` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OctoCode-serveren til å samhandle med OctoCode programmatisk.
:::

Denne arkitekturen lar OctoCode støtte flere klienter og lar deg samhandle med OctoCode programmatisk.

Du kan kjøre `octocode serve` for å starte en frittstående server. Hvis du har
OctoCode TUI kjører, vil `octocode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OctoCode [IDE](/docs/ide) plugins.
...
octocode-serveren viser følgende APIer.
