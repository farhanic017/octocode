---
title: Server
description: Interact with octocode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `octocode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an octocode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `octocode.local` |
...
Set `OCTOCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `octo`, or set `OCTOCODE_SERVER_USERNAME` to override it. This applies to both `octocode serve` and `octocode web`.
...
When you run `octo` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the octocode server to interact with octocode programmatically.
:::

This architecture lets octocode support multiple clients and allows you to interact with octocode programmatically.

You can run `octocode serve` to start a standalone server. If you have the
octocode TUI running, `octocode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OctoCode [IDE](/docs/ide) plugins.
...
The octocode server exposes the following APIs.
