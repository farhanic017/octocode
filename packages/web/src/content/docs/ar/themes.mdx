...
مع OctoCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OctoCode سمتنا `octo`.
...
يأتي OctoCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق OctoCode مظهر terminal لديهم
...
يدعم OctoCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/octocode/themes/*.json` أو `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.octocode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.octocode/themes/*.json`
