---
title: المزوّدون
description: استخدام أي موفّر LLM في OctoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OctoCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OctoCode.
...
في `~/.local/share/octocode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OctoCode.
...
OctoCode Zen هي قائمة نماذج يوفّرها فريق OctoCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OctoCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OctoCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر octocode، ثم انتقل إلى [octocode.ai/auth](https://octocode.ai/auth).
...
يعمل مثل أي مزوّد آخر في OctoCode واستخدامه اختياري بالكامل.

---

## OctoCode Go

OctoCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق OctoCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع OctoCode.

1. شغّل الأمر `/connect` في TUI، واختر `OctoCode Go`، ثم انتقل إلى [octocode.ai/auth](https://octocode.ai/zen).
...
لاستخدام Amazon Bedrock مع OctoCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل octocode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `octocode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OctoCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين octocode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل octocode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات octocode.
...
تخزين الرمز في مساحة تخزين مصادقة octocode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم OctoCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OctoCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`octocode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [octocode-gitlab-auth](https://www.npmjs.com/package/octocode-gitlab-auth).

##### التهيئة

خصّص عبر `octocode.json`:
...
لاستخدام اشتراك GitHub Copilot مع octocode:
...
لاستخدام Google Vertex AI مع OctoCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر octocode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [octocode-helicone-session](https://github.com/H2Shami/octocode-helicone-session) لتسجيل كل محادثة OctoCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OctoCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط octocode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط octocode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط octocode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OctoCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/octocode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OctoCode:
...
3. انسخ مفتاح API لاستخدامه في OctoCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في OctoCode، يجب سحب معلومات النموذج محليا:
...
OctoCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OctoCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OctoCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OctoCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OctoCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات octocode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات octocode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع OctoCode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات octocode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع octocode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `octocode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ OctoCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `octocode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات octocode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات octocode.
