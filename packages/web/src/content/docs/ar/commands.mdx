...
أنشئ الملف `.octocode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OctoCode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OctoCode:
...
- على مستوى النظام: `~/.config/octocode/commands/`
- لكل مشروع: `.octocode/commands/`
...
يتضمن octocode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
