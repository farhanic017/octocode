...
تتيح مهارات الوكيل لـ OctoCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث OctoCode في هذه المواقع:

- إعدادات المشروع: `.octocode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/octocode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج OctoCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.octocode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/octocode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.octocode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض OctoCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `octocode.json`:
...
**للوكلاء المدمجين** (في `octocode.json`):
