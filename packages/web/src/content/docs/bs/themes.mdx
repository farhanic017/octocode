...
U OctoCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OctoCode koristi nasu `octo` temu.
...
OctoCode dolazi sa vise ugradenih tema.
...
- Zele da OctoCode odgovara izgledu njihovog terminala
...
OctoCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/octocode/themes/*.json` ili `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.octocode/themes/*.json`
4. **Trenutni radni direktorij** - `./.octocode/themes/*.json`
