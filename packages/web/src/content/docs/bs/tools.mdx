...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OctoCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OctoCode.
...
:::note
Ovaj alat je dostupan samo kada je `OCTOCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `OCTOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OctoCode provajdera ili kada je varijabla `OCTOCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OctoCode:
