...
Da biste otklonili probleme s OctoCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\octocode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `octocode --log-level DEBUG`.

---

## Pohrana

OctoCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/octocode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\octocode`
...
OctoCode Desktop pokreće lokalni OctoCode server (`octocode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OctoCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/octocode/octocode.jsonc` (ili `~/.config/octocode/octocode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/octocode/octocode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\octocode\octocode.jsonc`
...
OctoCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/octocode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\octocode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OctoCode mogao ponovo izgraditi.

1. Potpuno zatvorite OctoCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/octocode`
- **Linux**: obrišite `~/.cache/octocode` (ili pokrenite `rm -rf ~/.cache/octocode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\octocode`

3. Ponovo pokrenite OctoCode Desktop.

---

### Rješavanje problema sa vezom na serveru

OctoCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `octocode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OCTOCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OCTOCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, OctoCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OctoCode karakteristikama.

---

### Obavještenja se ne prikazuju

OctoCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OctoCode u postavkama vašeg OS-a, i
...
1. Zatvorite OctoCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OctoCode Desktop aplikacije):

- `octocode.settings.dat` (URL zadanog servera za desktop)
- `octocode.global.dat` i `octocode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `octocode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\octocode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OctoCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\octocode`

2. Ponovo pokrenite OctoCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OctoCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OctoCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
