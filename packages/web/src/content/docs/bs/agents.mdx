...
Postoje dvije vrste agenata u OctoCode: primarni agenti i podagenti.
...
OctoCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
OctoCode dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

OctoCode dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u OctoCode-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `octocode.json`:
...
- Globalno: `~/.config/octocode/agents/`
- Po projektu: `.octocode/agents/`
...
Ako temperatura nije navedena, OctoCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu OctoCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj OctoCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OctoCode Zen](/docs/zen), koristili biste `octocode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `octocode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/octocode).
:::
