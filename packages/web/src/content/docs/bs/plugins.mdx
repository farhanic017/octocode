---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OctoCode.
---

Dodaci vam omogućavaju da proširite OctoCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OctoCode.
...
- `.octocode/plugins/` - Dodaci na nivou projekta
- `~/.config/octocode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/octocode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/octocode/octocode.json`)
2. Konfiguracija projekta (`octocode.json`)
3. Globalni direktorij dodataka (`~/.config/octocode/plugins/`)
4. Direktorij dodataka projekta (`.octocode/plugins/`)
...
OctoCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: OctoCode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OctoCode.
...
Spriječite octocode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u octocode:
...
Pomoćnik `tool` kreira prilagođeni alat koji octocode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za octocode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://octocode.ai/docs/sdk) za detalje.
