---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OctoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OctoCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OctoCode konfiguraciji.
...
u `~/.local/share/octocode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OctoCode
config.
...
OctoCode Zen je lista modela koje je obezbedio OctoCode tim koji su bili
testirano i potvrđeno da dobro radi sa OctoCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OctoCode Zen.
:::
...
Radi kao i svaki drugi provajder u OctoCode i potpuno je opcionalan za korištenje.

---

## OctoCode Go

OctoCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OctoCode tim i koji su testirani i verificirani da dobro rade s OctoCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OctoCode Go` i idite na [octocode.ai/auth](https://octocode.ai/zen).
...
Da biste koristili Amazon Bedrock s OctoCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete octocode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `octocode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OctoCode.
:::
...
Možete konfigurirati octocode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi octocode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju octocode konfiguraciju.
...
da pohrani token u octocode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OctoCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OctoCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`octocode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [octocode-gitlab-auth](https://www.npmjs.com/package/octocode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `octocode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s octocode:
...
Za korištenje Google Vertex AI s OctoCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće octocode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem octocodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [octocode-helicone-session](https://github.com/H2Shami/octocode-helicone-session) da automatski prijavite svaki OctoCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OctoCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati octocode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati octocode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati octocode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OctoCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/octocode) za detalje.
:::
...
Da biste koristili Ollama Cloud s OctoCode:
...
3. Kopirajte API ključ za korištenje u OctoCode.
...
6. **Važno**: Prije upotrebe modela oblaka u OctoCode, morate lokalno povući informacije o modelu:
...
OctoCode Zen je lista testiranih i verifikovanih modela koju je obezbedio OctoCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OctoCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OctoCode Zen**.

```txt
   /connect
```

3. Unesite svoj OctoCode API ključ.
...
Također možete dodati dodatne modele putem vaše octocode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše octocode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa OctoCode-om:
...
Također možete prilagoditi modele kroz svoju octocode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s octocode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `octocode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OctoCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `octocode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OctoCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj octocode konfiguraciji.
