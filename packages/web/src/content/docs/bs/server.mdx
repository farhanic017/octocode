---
title: Server
description: Komunicirajte s octocode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `octocode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji octocode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `octocode.local` |
...
Postavite `OCTOCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `octo`, ili postavite `OCTOCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `octocode serve` i za `octocode web`.
...
Kada pokrenete `octo`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite octocode server za programsku interakciju sa octocode-om.
:::

Ova arhitektura omogucava octocode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `octocode serve` da startate standalone server. Ako je octocode TUI vec pokrenut, `octocode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OctoCode [IDE](/docs/ide) pluginovi.
...
octocode server izlaže sljedece API-je.
