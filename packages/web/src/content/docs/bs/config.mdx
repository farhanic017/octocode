---
title: Konfiguracija
description: Korištenje OctoCode JSON konfiguracije.
---

Možete konfigurirati OctoCode koristeći JSON konfiguracijski fajl.

---

## Format

OctoCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/octocode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/octocode/octocode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OCTOCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`octocode.json` u projektu) - postavke specifične za projekat
5. **`.octocode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OCTOCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.octocode` i `~/.config/octocode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/octocode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu OctoCode konfiguraciju u `~/.config/octocode/octocode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/octocode/tui.json`.
...
Dodajte `octocode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se OctoCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OCTOCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OCTOCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.octocode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.octocode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`octocode.ai/config.json`**](https://octocode.ai/config.json).

TUI konfiguracija koristi [**`octocode.ai/tui.json`**](https://octocode.ai/tui.json).
...
Koristite `OCTOCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `octocode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `octocode serve` i `octocode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš OctoCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `octocode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj OctoCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, OctoCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/octocode/agents/` ili `.octocode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, OctoCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`octocode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/octocode/commands/` ili `.octocode/commands/`. [Saznajte više ovdje](/docs/commands).
...
OctoCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, OctoCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju OctoCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.octocode/plugins/` ili `~/.config/octocode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite OctoCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
