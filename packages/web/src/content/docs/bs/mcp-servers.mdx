...
Možete dodati vanjske alate u OctoCode koristeći _Model Context Protocol_, ili MCP. OctoCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OctoCode Config](https://octocode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/octocode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OctoCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OctoCode će:
...
Ako server zahtijeva autentifikaciju, OctoCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `octocode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OctoCode sigurno čuva tokene u `~/.local/share/octocode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OctoCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz OctoCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OctoCode sa vašim Sentry nalogom.
