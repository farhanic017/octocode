...
OctoCode Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u OctoCode-u. Pretplatite se na OctoCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili OctoCode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa OctoCode-om.
:::
...
OctoCode Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

OctoCode Go radi kao bilo koji drugi provajder u OctoCode-u.

1. Prijavite se na **<a href={console}>OctoCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `OctoCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na OctoCode Go.
:::
...
OctoCode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj OctoCode konfiguraciji
koristi format `octocode-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`octocode-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo OctoCode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz OctoCode koristite i bilo kojeg drugog provajdera.
