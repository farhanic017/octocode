...
Create `.octocode/commands/test.md`:
...
You can add custom commands through the OctoCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OctoCode [config](/docs/config):
...
octocode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
