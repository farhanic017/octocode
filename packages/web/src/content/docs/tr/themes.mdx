...
octocode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak octocode kendi `octo` temasını kullanır.
...
octocode birden fazla yerleşik temayla gelir.
...
- octocode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
octocode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/octocode/themes/*.json` veya `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.octocode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.octocode/themes/*.json`
