...
OctoCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, OctoCode'daki diğer sağlayıcılar gibi çalışır. OctoCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve OctoCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OctoCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

OctoCode Go, OctoCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OctoCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `OctoCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye OctoCode Go'ya abone olabilir.
:::
...
OctoCode Go aşağıdaki limitleri içerir:
...
OctoCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `octocode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `octocode-go/kimi-k2.6` kullanmalısınız.
...
OctoCode Go'yu şu amaçlarla oluşturduk:
...
4. OctoCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
