---
title: Eklentiler
description: OctoCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek OctoCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya OctoCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.octocode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/octocode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/octocode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/octocode/octocode.json`)
2. Project config (`octocode.json`)
3. Global eklenti dizini (`~/.config/octocode/plugins/`)
4. Proje eklenti dizini (`.octocode/plugins/`)
...
OctoCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir OctoCode SDK istemcisi.
...
OctoCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
OctoCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
OctoCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca OctoCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, OctoCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra OctoCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://octocode.ai/docs/sdk) bakın.
