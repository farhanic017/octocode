...
_Model Bağlam Protokolü_ veya MCP kullanarak octocode'a harici araçlar ekleyebilirsiniz. octocode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [octocode Config](https://octocode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/octocode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
octocode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde octocode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa octocode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `octocode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından octocode, belirteçleri `~/.local/share/octocode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra octocode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi octocode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve octocode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
