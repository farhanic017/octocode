...
octocode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\octocode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `octocode --log-level DEBUG`.

---

## Depolama

octocode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/octocode/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\octocode` yapistirin
...
octocode Desktop arka planda yerel bir octocode sunucusu (`octocode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `octo` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/octocode/octocode.jsonc` (veya `~/.config/octocode/octocode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/octocode/octocode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\octocode\octocode.jsonc` yapistirin
...
octocode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/octocode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\octocode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), octocode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. octocode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/octocode`
- **Linux**: `~/.cache/octocode` dizinini silin (veya `rm -rf ~/.cache/octocode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\octocode` yapistirin

3. octocode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

octocode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`octocode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `OCTOCODE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `OCTOCODE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta octocode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, octocode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

octocode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda octocode icin bildirimler etkinse
...
1. octocode Desktop'u kapatin
2. Su dosyalari bulun ve silin (octocode Desktop uygulama veri dizininde yer alirlar):

- `octocode.settings.dat` (desktop varsayilan sunucu URL'si)
- `octocode.global.dat` ve `octocode.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
octocode ile ilgili bir sorun yasiyorsaniz:
...
3. `octocode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `octocode models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\octocode`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. octocode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\octocode`

2. En guncel provider paketlerini yeniden kurmak icin octocode'u yeniden baslatin

Bu, octocode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
octocode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
