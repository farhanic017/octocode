---
title: Sunucu
description: octocode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`octocode serve` komutu, octocode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `octocode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OCTOCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `octo` değeridir; değiştirmek isterseniz `OCTOCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `octocode serve` hem de `octocode web` için geçerlidir.
...
`octo` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
octocode ile programatik etkileşim için octocode sunucusunu kullanın.
:::

Bu mimari, octocode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`octocode serve` ile bağımsız bir sunucu başlatabilirsiniz. octocode TUI açıksa, `octocode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum octocode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
octocode sunucusu aşağıdaki API'leri sunar.
