...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. octocode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
octocode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `OCTOCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `OCTOCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca octocode provider kullanılırken veya `OCTOCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

octocode başlatırken etkinleştirmek için:
