...
description: OctoCode, LSP sunucularınızla bütünleşir.
---

OctoCode, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

OctoCode, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`OCTOCODE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve octocode bir dosyayı açtığında:
...
LSP sunucularını, octocode yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
