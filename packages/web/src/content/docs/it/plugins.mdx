---
title: Plugin
description: Scrivi plugin per estendere OctoCode.
---

I plugin ti permettono di estendere OctoCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di OctoCode.
...
- `.octocode/plugins/` - plugin a livello progetto
- `~/.config/octocode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/octocode/node_modules/`.
...
1. Config globale (`~/.config/octocode/octocode.json`)
2. Config di progetto (`octocode.json`)
3. Directory plugin globale (`~/.config/octocode/plugins/`)
4. Directory plugin di progetto (`.octocode/plugins/`)
...
OctoCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di octocode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere octocode.
...
:::note
Se usi l'app desktop di OctoCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a octocode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a octocode:
...
L'helper `tool` crea uno strumento personalizzato che octocode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in octocode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://octocode.ai/docs/sdk) per i dettagli.
