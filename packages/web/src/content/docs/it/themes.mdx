...
Con OctoCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, OctoCode usa il tema `octo`.
...
OctoCode include diversi temi integrati.
...
- Vuole che OctoCode corrisponda all'aspetto del terminale
...
OctoCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/octocode/themes/*.json` o `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.octocode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.octocode/themes/*.json`
