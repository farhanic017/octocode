...
Le skill degli agenti permettono a OctoCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OctoCode cerca in queste posizioni:

- Config di progetto: `.octocode/skills/<name>/SKILL.md`
- Config globale: `~/.config/octocode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OctoCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.octocode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/octocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.octocode/skills/git-release/SKILL.md` cosi':
...
OctoCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `octocode.json`:
...
**Per agenti integrati** (in `octocode.json`):
