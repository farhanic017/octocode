...
In OctoCode esistono due tipi di agenti: agenti primari e subagenti.
...
OctoCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OctoCode include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

OctoCode include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di OctoCode, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `octocode.json`:
...
- Globale: `~/.config/octocode/agents/`
- Per progetto: `.octocode/agents/`
...
Se non specifichi una temperature, OctoCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OctoCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione OctoCode usa il formato `provider/model-id`. Per esempio, se stai usando [OctoCode Zen](/docs/zen), useresti `octocode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `octocode models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/octocode).
:::
