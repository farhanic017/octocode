---
title: Server
description: Interagisci con il server octocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `octocode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client octocode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `octocode.local` |
...
Imposta `OCTOCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `octo`, oppure imposta `OCTOCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `octocode serve` sia per `octocode web`.
...
Quando esegui `octo` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server octocode per interagire con octocode in modo programmatico.
:::

Questa architettura permette a octocode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `octocode serve` per avviare un server standalone. Se la TUI di octocode e gia in esecuzione, `octocode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di OctoCode.
...
Il server octocode espone le seguenti API.
