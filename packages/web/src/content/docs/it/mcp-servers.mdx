...
Puoi aggiungere strumenti esterni a OctoCode usando il _Model Context Protocol_, o MCP. OctoCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [OctoCode Config](https://octocode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/octocode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
OctoCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, OctoCode:
...
Se il server richiede autenticazione, OctoCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `octocode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, OctoCode memorizzera' i token in modo sicuro in `~/.local/share/octocode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in OctoCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione OctoCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare OctoCode al tuo account Sentry.
