...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. OctoCode include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in OctoCode.
...
:::note
Questo strumento e' disponibile solo quando `OCTOCODE_EXPERIMENTAL_LSP_TOOL=true` (oppure `OCTOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OctoCode o quando la variabile d'ambiente `OCTOCODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di OctoCode:
