...
OctoCode Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in OctoCode. Ti abboni a OctoCode Go e
...
OctoCode Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

OctoCode Go funziona come qualsiasi altro provider in OctoCode.

1. Accedi a **<a href={console}>OctoCode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `OctoCode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a OctoCode Go.
:::
...
OctoCode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua OctoCode config
utilizza il formato `octocode-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`octocode-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato OctoCode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con OctoCode.
