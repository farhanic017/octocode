---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OctoCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OctoCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OctoCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di OctoCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OctoCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OctoCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `octocode.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo OctoCode attende dopo il leader key. Default: `2000`.
...
Usa `OCTOCODE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
...
- `sound_pack` - ID del sound pack da usare. Default: `octocode.default`.
