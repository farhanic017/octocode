...
Med OctoCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OctoCode vårt eget `octo`-tema.
...
OctoCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/octocode/themes/*.json` eller `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.octocode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.octocode/themes/*.json`
