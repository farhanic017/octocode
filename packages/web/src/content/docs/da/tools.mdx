...
Her er alle de innebygde verktøyene som er tilgængelige i OctoCode.
...
:::note
Dette verktøyet er bare tilgængelig når `OCTOCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `OCTOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OctoCode-leverandøren eller når miljøvariabelen `OCTOCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
