...
For at fejlfinde problemer med OctoCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\octocode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `octocode --log-level DEBUG`.

---

## Lagring

octocode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/octocode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\octocode`
...
OctoCode Desktop kører en lokal OctoCode-server (`octocode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `OctoCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/octocode/octocode.jsonc` (eller `~/.config/octocode/octocode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/octocode/octocode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\octocode\octocode.jsonc`
...
OctoCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/octocode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\octocode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så OctoCode kan genopbygge den.

1. Afslut OctoCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/octocode`
- **Linux**: slet `~/.cache/octocode` (eller kør `rm -rf ~/.cache/octocode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\octocode`

3. Start OctoCode Desktop på ny.

---

### Løs problemer med serverforbindelse

OctoCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `octocode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `OCTOCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OCTOCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver OctoCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i OctoCode.

---

### Meddelelser vises ikke

OctoCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for OctoCode i dine OS-indstillinger, og
...
1. Afslut OctoCode Desktop.
2. Find og slet disse filer (de findes i OctoCode Desktop-appens datamappe):

- `octocode.settings.dat` (skrivebordsstandardserver URL)
- `octocode.global.dat` og `octocode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med OctoCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `octocode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\octocode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. octocode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\octocode`

2. Start octocode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge octocode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
octocode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
