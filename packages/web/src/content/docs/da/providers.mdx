---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OctoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OctoCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OctoCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/octocode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OctoCode-konfigurationen.
...
OctoCode Zen er en liste over modeller leveret af OctoCode-teamet, som er blevet testet og verificeret til at fungere godt med OctoCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OctoCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg octocode og gå til [octocode.ai/auth](https://octocode.ai/auth).
...
Det fungerer som alle andre udbydere i OctoCode og er helt valgfrit at bruge.

---

## OctoCode Go

OctoCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OctoCode-teamet, som er testet og verificeret til at fungere godt med OctoCode.

1. Kør kommandoen `/connect` i TUI, vælg `OctoCode Go`, og gå til [octocode.ai/auth](https://octocode.ai/zen).
...
Sådan bruger du Amazon Bedrock med OctoCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører octocode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `octocode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OctoCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere octocode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at octocode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem octocode-konfigurationen.
...
gemme tokenet i octocode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OctoCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OctoCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`octocode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [octocode-gitlab-auth](https://www.npmjs.com/package/octocode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `octocode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med OctoCode:
...
Sådan bruger du Google Vertex AI med OctoCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører octocode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem octocode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [octocode-helicone-session](https://github.com/H2Shami/octocode-helicone-session) plugin for automatisk at logge hver OctoCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OctoCode-samtale opført som en separat session.
...
Du kan konfigurere octocode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere octocode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere octocode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OctoCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/octocode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OctoCode:
...
3. Kopier API-nøglen til brug i OctoCode.
...
6. **Vigtigt**: Før du bruger skymodeller i OctoCode, skal du hente modelinformationen lokalt:
...
OctoCode Zen er en liste over testede og verificerede modeller leveret af OctoCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OctoCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OctoCode Zen**.

   ```txt
   /connect
   ```

3. Indtast OctoCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem octocode-konfigurationen.
...
5. Du kan også tilpasse dem gennem octocode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OctoCode:
...
Du kan også tilpasse modeller gennem octocode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med octocode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `octocode.json`-filen i projektkataloget:
...
`limit`-felterne lader OctoCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `octocode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek octocode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i octocode-konfigurationen.
