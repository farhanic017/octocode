---
title: Regeln
description: Lege eigene Anweisungen fuer octocode fest.
---

Du kannst octocode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in octocode den Befehl `/init` aus.
...
So kann octocode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht octocode sie zu erweitern.
...
octocode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/octocode/AGENTS.md` speichern.
Diese gelten in allen octocode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OctoCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/octocode/AGENTS.md` existiert)
...
Beim Start sucht octocode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/octocode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `octocode.json` oder in `~/.config/octocode/octocode.json` angeben.
...
octocode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit octocode.json

Empfohlen ist das `instructions`-Feld in der `octocode.json`:
...
Du kannst octocode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `octocode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
