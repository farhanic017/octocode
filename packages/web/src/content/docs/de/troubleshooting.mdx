...
Wenn OctoCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\octocode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `octocode --log-level DEBUG`.

---

## Speicher

octocode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/octocode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\octocode`
...
OctoCode Desktop startet im Hintergrund einen lokalen OctoCode-Server (`octocode-cli`-Sidecar).
...
- Nur macOS: `OctoCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/octocode/octocode.jsonc` (oder `~/.config/octocode/octocode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/octocode/octocode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\octocode\octocode.jsonc`
...
OctoCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/octocode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\octocode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit OctoCode ihn neu aufbauen kann.

1. Beende OctoCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/octocode`
- **Linux**: loesche `~/.cache/octocode` (oder fuehre aus: `rm -rf ~/.cache/octocode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\octocode`

3. Starte OctoCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

OctoCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `octocode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OCTOCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OCTOCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt OctoCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit OctoCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

OctoCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer OctoCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende OctoCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von OctoCode Desktop):

- `octocode.settings.dat` (desktop default server URL)
- `octocode.global.dat` und `octocode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit OctoCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `octocode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\octocode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. octocode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\octocode`

2. Starte octocode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt octocode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
octocode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
