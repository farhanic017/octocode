...
Mit OctoCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet OctoCode das `octo`-Theme.
...
OctoCode liefert mehrere eingebaute Themes mit.
...
- OctoCode optisch ans Terminal anpassen wollen
...
OctoCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/octocode/themes/*.json` oder `$XDG_CONFIG_HOME/octocode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.octocode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.octocode/themes/*.json`
