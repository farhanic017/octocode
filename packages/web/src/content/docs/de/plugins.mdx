---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OctoCode zu erweitern.
---

Mit Plugins können Sie OctoCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OctoCode zu ändern.
...
- `.octocode/plugins/` – Plugins auf Projektebene
- `~/.config/octocode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/octocode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/octocode/octocode.json`)
2. Projektkonfiguration (`octocode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/octocode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.octocode/plugins/`)
...
OctoCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OctoCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OctoCode erweitern können.
...
:::note
Wenn Sie die OctoCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OctoCode `.env`-Dateien liest:
...
Plugins können OctoCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OctoCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OctoCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://octocode.ai/docs/sdk).
