...
OctoCode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in OctoCode. Du abonnierst OctoCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
OctoCode zu verwenden.
...
OctoCode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

OctoCode Go funktioniert wie jeder andere Provider in OctoCode.

1. Du meldest dich bei **<a href={console}>OctoCode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `OctoCode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann OctoCode Go abonnieren.
:::
...
OctoCode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner OctoCode Config verwendet das Format `octocode-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `octocode-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben OctoCode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit OctoCode zu nutzen.
