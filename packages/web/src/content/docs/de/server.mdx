---
title: Server
description: Nutze den octocode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `octocode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein octocode-Client nutzen kann.
...
Setze `OCTOCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `octo`, kann aber mit `OCTOCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `octocode serve` und `octocode web`.
...
Wenn du `octo` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den octocode-Server, um octocode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `octocode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `octocode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die OctoCode-[IDE](/docs/ide)-Plugins.
...
Der octocode-Server stellt folgende APIs bereit.
