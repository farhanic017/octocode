...
Agent Skills erlauben OctoCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OctoCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OctoCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.octocode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/octocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.octocode/skills/git-release/SKILL.md` zum Beispiel so:
...
OctoCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `octocode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `octocode.json`):
