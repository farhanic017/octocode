...
import { getOctocodeModeStack, OCTOCODE_BASE_MODE, OctocodeKeymapProvider, registerOctocodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerOctocodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when octocode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getOctocodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getOctocodeModeStack(keymap).push("autocomplete")
