...
export const OCTOCODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OCTOCODE_MODE_KEY = "octocode.mode"

export const OctocodeKeymapProvider = KeymapProvider
export const useOctocodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OctoTuiKeymap = ReturnType<typeof useKeymap>
type OctocodeModeStack = ReturnType<typeof createOctocodeModeStack>
...
type Command = ReturnType<OctoTuiKeymap["getCommands"]>[number]
type BindingLookup = {
...
const modeStacks = new WeakMap<OctoTuiKeymap, OctocodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOctocodeModeStack(keymap: OctoTuiKeymap) {
  keymap.setData(OCTOCODE_MODE_KEY, OCTOCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OCTOCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OCTOCODE_MODE_KEY, stack.at(-1)?.mode ?? OCTOCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OCTOCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OCTOCODE_MODE_KEY, undefined)
...
export function useOctocodeModeStack() {
  return getOctocodeModeStack(useOctocodeKeymap())
}

export function getOctocodeModeStack(keymap: OctoTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Octocode mode stack is not registered for this keymap")
  return value
}
...
function registerKeyAliases(keymap: OctoTuiKeymap) {
...
export function registerOctocodeKeymap(keymap: OctoTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createOctocodeModeStack(keymap)
...
export function useLeaderActive(): Accessor<boolean> {
  return useKeymapSelector((keymap: OctoTuiKeymap) => keymap.getPendingSequence()[0]?.tokenName === LEADER_TOKEN)
}

export function useCommandShortcut(command: string): Accessor<string> {
  const config = useTuiConfig()
  return useKeymapSelector((keymap: OctoTuiKeymap) =>
    formatKeySequence(
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  ...
  const entries = useKeymapSelector((keymap: OctoTuiKeymap) =>
