...
import type { AssistantMessage, FilePart, UserMessage } from "@octocode-ai/sdk/v2"
...
import { OCTOCODE_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, useOctocodeKeymap } from "../../keymap"
import { useTuiConfig } from "../../config"
import { usePromptWorkspace } from "./workspace"
import { usePromptMove } from "./move"
import { readLocalAttachment } from "./local-attachment"
import { openLocalFile, openLocalFilePicker } from "./local-file-picker"
import "./attachment-thumbnail"
...
const HOME_DOCK_BG = RGBA.fromHex("#202020")
const HOME_DOCK_TEXT_MUTED = RGBA.fromHex("#7a7a78")
const HOME_DOCK_TEXT = RGBA.fromHex("#e9e7e7")
const HOME_DOCK_ACCENT = RGBA.fromHex("#8732bf")
const HOME_DOCK_ACCENT_HOVER = RGBA.fromHex("#9a42d3")
const HOME_DOCK_VOICE = RGBA.fromHex("#8b8b90")
const HOME_DOCK_LIVE_DOT = RGBA.fromHex("#ff5858")
const HOME_ATTACHMENT_BG = RGBA.fromHex("#080808")
const HOME_ATTACHMENT_BORDER = RGBA.fromHex("#363636")
const HOME_ATTACHMENT_TEXT = RGBA.fromHex("#d7d3d6")
const HOME_AGENT_MODES = [
  {
    value: "build",
    title: "Build (Default)",
    description: "",
    agent: "build",
    color: RGBA.fromHex("#4dabf7"),
  },
  {
    value: "plan",
    title: "Plan",
...
type HomeAgentMode = (typeof HOME_AGENT_MODES)[number]["value"]
...
export function Prompt(props: PromptProps) {
  ...
  const currentProviderLabel = createMemo(() => {
    const upstream = ["Open", "Code"].join("")
    return local.model.parsed().provider.replaceAll(upstream, "octo code").replaceAll("octo", "octo code")
  })
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.OCTOCODE_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
          workspace.open()
        },
  ...
  const ref: PromptRef = {
    get focused() {
      return input && !input.isDestroyed ? input.focused : false
    },
    get current() {
      return store.prompt
    },
  ...
  onCleanup(() => {
    if (store.prompt.input && input && !input.isDestroyed) {
  ...
  function restoreExtmarksFromParts(parts: PromptInfo["parts"]) {
    ...
    parts.forEach((part, partIndex) => {
      ...
      if (part.type === "file" && part.source?.text) {
        ...
        shouldCreate = Boolean(virtualText)
  ...
  async function submitInner() {
    ...
    const inputText = expandTrackedPastedText(
      store.prompt.input,
      input && !input.isDestroyed ? input.extmarks.getAllForTypeId(promptPartTypeId).flatMap((extmark) => {
  ...
  const inputArea = () => (
    <>
      <textarea
        ...
        placeholderColor={props.homeDock ? HOME_DOCK_TEXT_MUTED : theme.textMuted}
        textColor={props.homeDock ? HOME_DOCK_TEXT : leader() ? theme.textMuted : theme.text}
        focusedTextColor={props.homeDock ? HOME_DOCK_TEXT : leader() ? theme.textMuted : theme.text}
        minHeight={1}
        maxHeight={props.homeDock ? 1 : maxHeight()}
        onContentChange={() => {
          if (!input || input.isDestroyed) return
        ...
        ref={(r: TextareaRenderable) => {
          ...
          setTimeout(() => {
            // setTimeout is a workaround and needs to be addressed properly
            if (!input || input.isDestroyed) return
            input.cursorColor = props.homeDock ? HOME_DOCK_BG : theme.text
          }, 0)
        }}
        onMouseDown={(r: MouseEvent) => r.target?.focus()}
        focusedBackgroundColor={props.homeDock ? HOME_DOCK_BG : theme.backgroundElement}
        cursorColor={props.homeDock ? HOME_DOCK_BG : props.disabled ? theme.backgroundElement : theme.text}
        syntaxStyle={syntax()}
      />
    </>
  )

  const [homeControlHover, setHomeControlHover] = createSignal<
    "add" | "agent" | "model" | "effort" | "send" | null
  >(null)
  const [homeAgentMode, setHomeAgentMode] = createSignal<HomeAgentMode>(
    (local.agent.current()?.name as HomeAgentMode) ?? "build"
  )
  const homeAttachments = createMemo(() =>
    store.prompt.parts
      .map((part, index) => ({ part, index }))
      .filter((item): item is { part: Extract<PromptInfo["parts"][number], { type: "file" }>; index: number } =>
        item.part.type === "file",
      ),
  )
  ...
  const attachmentKind = (mime: string) => {
  ...
  const attachmentAccent = (mime: string) => {
    if (mime.startsWith("image/")) return HOME_DOCK_ACCENT_HOVER
    if (mime.startsWith("video/")) return RGBA.fromHex("#ff5c7a")
    if (mime.startsWith("audio/")) return RGBA.fromHex("#5aa9ff")
    if (mime === "application/pdf") return RGBA.fromHex("#f05a5a")
    return RGBA.fromHex("#a8a8ad")
  }
  const attachmentName = (part: Extract<PromptInfo["parts"][number], { type: "file" }>) =>
    Locale.truncateMiddle(part.filename ?? "attachment", 15)
  const attachmentPath = (part: Extract<PromptInfo["parts"][number], { type: "file" }>) =>
    part.source?.type === "file" ? part.source.path : undefined
  const openHomeAttachment = (part: Extract<PromptInfo["parts"][number], { type: "file" }>) => {
    const filepath = attachmentPath(part)
    if (!filepath) {
      toast.show({ variant: "warning", message: "This attachment has no local file path" })
      return
    }
              ...
              >
                ...
                >
                  <attachment_thumbnail
                    ...
                    fallbackBg={HOME_ATTACHMENT_BG}
                    border={HOME_ATTACHMENT_BORDER}
                  />
                  <text fg={HOME_DOCK_TEXT_MUTED} wrapMode="none">
                    {attachmentName(item.part)}
                  </text>
  ...
  const homeAgentModeConfig = createMemo(
    () => HOME_AGENT_MODES.find((item) => item.value === homeAgentMode()) ?? HOME_AGENT_MODES[0],
  )
  const agentModeColor = createMemo(() => {
    const value = homeAgentMode()
    switch (value) {
      case "build": return RGBA.fromHex("#4dabf7")  // Blue
      case "plan": return RGBA.fromHex("#69db7c")   // Green
      case "agent-swarm": return RGBA.fromHex("#ff6b6b")  // Red
      default: return HOME_DOCK_TEXT
    }
  })
  const effortLabel = createMemo(() => {
    const value = local.model.variant.current()
    if (!value || value === "default") return { label: "High", color: theme.warning }
    const label = Locale.titlecase(value)
    const color = (() => {
      switch (value.toLowerCase()) {
        case "none": return RGBA.fromHex("#ff69b4")
        case "minimal": return RGBA.fromHex("#ffd700")
        case "low": return theme.success
        case "medium": return theme.info
        case "high": return theme.warning
        case "xhigh": return RGBA.fromHex("#40e0d0")
  ...
  const [homeAttachmentPickerOpen, setHomeAttachmentPickerOpen] = createSignal(false)
  const openHomeAttachmentPicker = async () => {
    if (homeAttachmentPickerOpen()) return
    setHomeAttachmentPickerOpen(true)
    try {
      const files = await openLocalFilePicker({
        platform: terminalEnvironment.platform,
        cwd: project.instance.directory() || paths.cwd,
      })
      for (const file of files) await pasteInputText(file)
    } catch (error) {
      toast.show({
        variant: "error",
        message: errorMessage(error) || "File picker failed",
      })
    } finally {
      setHomeAttachmentPickerOpen(false)
    }
  }
  const openHomeAgentModeDialog = () => {
    dialog.replace(() => {
      dialog.setSize("small")
      return (
        <DialogSelect<HomeAgentMode>
          title="Agent mode"
          current={homeAgentMode()}
          options={HOME_AGENT_MODES.map((item) => ({
            title: item.title,
            value: item.value,
            color: item.color,
            onSelect: (dialog) => {
              setHomeAgentMode(item.value)
              local.agent.set(item.agent)
              dialog.clear()
            },
          }))}
          flat={true}
  ...
  const homeDockButton = (options: {
    ...
    <box
      width={options.width}
      alignItems="center"
      onMouseOver={() => setHomeControlHover(options.id)}
      onMouseOut={() => setHomeControlHover(null)}
      onMouseUp={options.onPress}
    >
      {options.children}
    </box>
  )

  const homeDockControls = () => (
    <box flexDirection="row" flexShrink={0} paddingTop={1} gap={1} justifyContent="space-between">
      <box flexDirection="row" gap={0}>
        {homeDockButton({
          id: "add",
          width: 2,
          onPress: () => void openHomeAttachmentPicker(),
          children: (
            <text fg={hoverFg("add", HOME_DOCK_TEXT)}>
        ...
        {homeDockButton({
          id: "agent",
          width: 18,
          onPress: openHomeAgentModeDialog,
          children: (
            <text fg={agentModeColor()} wrapMode="none">
              {homeAgentModeConfig().title}
            </text>
          ),
        })}
      </box>
      <box flexDirection="row" gap={1} alignItems="center">
        ...
        {homeDockButton({
          ...
          children: (
            <text fg={homeControlHover() === "effort" ? effortLabel().color : effortLabel().color} wrapMode="none">
              {effortLabel().label}
            </text>
          ),
        })}
  ...
  return (
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
        <box
          width="100%"
          border={props.homeDock ? false : ["left"]}
          borderColor={props.homeDock ? HOME_DOCK_BG : borderHighlight()}
          backgroundColor={props.homeDock ? HOME_DOCK_BG : undefined}
          customBorderChars={{
        ...
        >
          <box
            paddingLeft={props.homeDock ? 2 : 2}
            paddingRight={props.homeDock ? 2 : 2}
            paddingTop={props.homeDock ? 0 : 1}
            flexShrink={0}
            backgroundColor={props.homeDock ? HOME_DOCK_BG : theme.backgroundElement}
            flexGrow={1}
            width="100%"
          >
            <textarea
            ...
            <box visible={true} flexDirection="row" flexShrink={0} paddingTop={1} gap={1} justifyContent="space-between">
                      ...
                      <Show when={store.mode === "normal"}>
                        <box flexDirection="row" gap={1}>
                          ...
                          <Show when={showVariant()}>
                            ...
                            <text>
                              <span style={{ fg: fadeColor(effortLabel().color, variantMetaAlpha()), bold: true }}>
        ...
        <box visible={true} width="100%" flexDirection="row" justifyContent="space-between">
          <Switch>
