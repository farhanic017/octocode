import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@octocode-ai/plugin/tui"
...
import type { useOctocodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useOctocodeKeymap>
...
function routeCurrent(route: ReturnType<typeof useRoute>): TuiPluginApi["route"]["current"] {
  ...
  if (route.data.type === "welcome") return { name: "welcome" }
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getOctocodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOctocodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
