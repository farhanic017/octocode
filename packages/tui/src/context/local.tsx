...
import { readJson, writeJsonAtomic, writeJsonSync } from "../util/persistence"
...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    function createAgent() {
      const HOME_AGENT_NAMES = ["build", "plan", "agent-swarm"]
      const agents = createMemo(() => sync.data.agent.filter((x) => x.mode !== "subagent" && !x.hidden))
      const visibleAgents = createMemo(() => sync.data.agent.filter((x) => !x.hidden))
      const [agentStore, setAgentStore] = createStore({
        current: "build" as string | undefined,
      })
      ...
      // Fixed list of home agent modes for tab cycling
      const homeAgentModes = [
        { name: "build", color: RGBA.fromHex("#4dabf7") },
        { name: "plan", color: RGBA.fromHex("#69db7c") },
        { name: "agent-swarm", color: RGBA.fromHex("#ff6b6b") },
      ]
      ...
      return {
        ...
        set(name: string) {
          // Allow setting any of the home agent modes
          if (HOME_AGENT_NAMES.includes(name)) {
        ...
        move(direction: 1 | -1) {
          batch(() => {
            // Use home agent modes for cycling
            const currentIndex = homeAgentModes.findIndex((x) => x.name === agentStore.current)
            let next = currentIndex + direction
            if (next < 0) next = homeAgentModes.length - 1
            if (next >= homeAgentModes.length) next = 0
            const value = homeAgentModes[next]
            setAgentStore("current", value.name)
          })
        },
        color(name: string) {
          // Use predefined colors for home agent modes
          const homeMode = homeAgentModes.find((x) => x.name === name)
          if (homeMode) return homeMode.color
    ...
    function createModel() {
      ...
      return {
        ...
        cycleHidden(direction: 1 | -1) {
          const hidden = modelStore.hidden.filter((item) => isModelValid(item))
          if (!hidden.length) {
            toast.show({
              variant: "info",
              message: "No hidden models",
              duration: 3000,
            })
            return
          }
          ...
          let index = -1
          if (current) {
            index = hidden.findIndex((x) => x.providerID === current.providerID && x.modelID === current.modelID)
          }
          if (index === -1) {
            index = direction === 1 ? 0 : hidden.length - 1
          } else {
            index += direction
            if (index < 0) index = hidden.length - 1
            if (index >= hidden.length) index = 0
          }
