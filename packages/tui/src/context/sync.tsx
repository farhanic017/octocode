...
export const {
  ...
  init: () => {
    ...
    const deltaBuffer = new Map<string, Map<string, { field: string; delta: string }[]>>()
      ...
      switch (event.type) {
        ...
        case "message.part.delta": {
          ...
          if (!parts) {
            ...
            const list = buf.get(event.properties.partID) ?? []
            list.push({ field: event.properties.field, delta: event.properties.delta })
            buf.set(event.properties.partID, list)
