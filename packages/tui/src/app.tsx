    ...
    Effect.gen(function* () {
      const renderer = yield* Effect.acquireRelease(
        Effect.tryPromise(async () => {
          try {
            console.error("TUI: Creating renderer...")
            const r = await createCliRenderer({
              externalOutputMode: "passthrough",
              targetFps: 60,
              gatherStats: false,
              exitOnCtrlC: false,
              useKittyKeyboard: process.platform === "darwin" ? {} : undefined,
              autoFocus: false,
              openConsoleOnError: false,
              useMouse: !Flag.OCTOCODE_DISABLE_MOUSE && input.config.mouse,
              consoleOptions: {
                keyBindings: [{ name: "y", ctrl: true, action: "copy-selection" }],
              },
            })
            console.error("TUI: Renderer created successfully")
            return r
          } catch (error) {
            console.error("TUI: Failed to create renderer:", error)
            throw error
          }
        }),
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerOctocodeKeymap(keymap, renderer, input.config)),
      ...
      yield* Effect.tryPromise(async () => {
        ...
        if (renderer.isDestroyed) {
          console.error("Renderer was destroyed before rendering")
          return
        }

        console.error("TUI: Starting render with mode:", mode)
        try {
          await render(() => {
          return (
            <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
              ...
              >
                ...
                >
                  <TuiStartupProvider
                    value={{
                      initialRoute: process.env.OCTOCODE_ROUTE ? JSON.parse(process.env.OCTOCODE_ROUTE) : undefined,
                      skipInitialLoading: Boolean(process.env.OCTOCODE_FAST_BOOT),
                    }}
                  >
                    <ClipboardProvider>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`octo | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`octo | ${route.data.id}`)
    }
  })
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.OCTOCODE_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          toast.show({ message: "octo code docs are not wired yet", variant: "info" })
          dialog.clear()
        },
        category: "System",
      },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to octo code v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.OCTOCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.OCTOCODE_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
