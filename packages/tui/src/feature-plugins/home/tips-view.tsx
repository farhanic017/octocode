import type { TuiPluginApi } from "@octocode-ai/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("octocode.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at octo code share service",
  ...
  "Create {highlight}octo.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/octo/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.octo/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.octo/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.octo/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.octo/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent octo code from reading sensitive files",
  "Use {highlight}octo run{/highlight} for non-interactive scripting",
  "Use {highlight}octo --continue{/highlight} to resume the last session",
  "Use {highlight}octo run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}octo serve{/highlight} for headless API access to octo code",
  "Use {highlight}octo run --attach{/highlight} to connect to a running server",
  "Run {highlight}octo upgrade{/highlight} to update to the latest version",
  "Run {highlight}octo auth list{/highlight} to see all configured providers",
  "Run {highlight}octo agent create{/highlight} for guided agent creation",
  "Use {highlight}/octo{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}octo github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/octo fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.octo/themes/{/highlight} directory",
  ...
  "Run {highlight}octo debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm octo code container image{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with octo code Zen for curated, tested models",
