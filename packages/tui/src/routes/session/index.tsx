...
import { OCTOCODE_BASE_MODE, useBindings, useCommandShortcut, useOctocodeKeymap } from "../../keymap"
...
const GO_UPSELL_PROVIDERS = new Set(["octo", "octocode-go"])
...
export function Session() {
  ...
  createEffect(() => {
    ...
    void (async () => {
      ...
      if (route.sessionID === sessionID && scrollOk()) scroll.scrollBy(100_000)
    })().catch((error) => {
  ...
  function scrollOk() {
    return Boolean(scroll) && !scroll.isDestroyed
  }
        ...
        <box flexDirection="row" flexGrow={1} minHeight={0}>
          <box flexGrow={1} minHeight={0} paddingBottom={1} gap={1}>
            <Show when={showTopNav()}>
              <box height={2} flexDirection="row" justifyContent="space-between" gap={1} flexShrink={0} paddingLeft={2} paddingRight={3}>
                <box flexDirection="row" gap={1} alignItems="center">
                  <Show when={sessionStatus().type !== "idle"}>
                    <box
                      onMouseUp={handleInterrupt}
                      flexDirection="row"
                      gap={1}
                      alignItems="center"
                    >
                ...
                <box
                  ...
                  gap={1}
                  alignItems="center"
                  onMouseUp={() => keymap.dispatchCommand("docs.open")}
                >
                  <text fg={theme.text} attributes={TextAttributes.BOLD}>@</text>
                  <text fg={theme.textMuted}>Browser</text>
                </box>
                <box
                  flexDirection="row"
                  gap={1}
                ...
                <Show when={topNavUsage()}>
                  {(tokens) => (
                    <box
                      ...
                      alignItems="center"
                    >
                      <text fg={theme.textMuted}>{Locale.number(tokens())}</text>
                    </box>
                  )}
                </Show>
                <Show when={session()?.cost ?? 0}>
                  <box
                    ...
                    alignItems="center"
                  >
                    <text fg={theme.textMuted}>{Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(session()?.cost ?? 0)}</text>
                  </box>
                </Show>
                <Show when={topNavSubagents() > 0}>
                  <box
                    alignItems="center"
                    ...
                    height={1}
                  >
                    <text fg={theme.text}>{topNavSubagents()}</text>
                  </box>
                </Show>
                </box>
              </box>
