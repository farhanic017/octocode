...
export interface DialogSelectProps<T> {
  ...
  locked?: boolean
  maxHeight?: number
  filterButtons?: { label: string; active: boolean; onSelect: () => void }[]
  actions?: {
    command: string
    title: string
    side?: "left" | "right"
    ...
    type?: "button" | "toggle"
    checked?: boolean | ((option: DialogSelectOption<T> | undefined) => boolean)
...
export interface DialogSelectOption<T = any> {
  ...
  footerAfterDescription?: boolean
  titleWidth?: number
  ...
  rowActions?: { checked: () => boolean; onTrigger: () => void; icon?: string; display?: boolean }[]
}
...
export function DialogSelect<T>(props: DialogSelectProps<T>) {
  ...
  const grouped = createMemo<[string, DialogSelectOption<T>[]][]>(() => {
    ...
    try {
      ...
      console.error("grouped memo failed:", error)
      return [["", filtered()]]
    }
  })
  ...
  const height = createMemo(() => props.maxHeight ?? Math.floor(dimensions().height / 2) - 2)
  ...
  function FooterAction(action: { item: VisibleAction }) {
    ...
    if (item.type === "toggle") {
      const checked = createMemo(() => {
        const val = typeof item.checked === "function" ? item.checked(selected()) : item.checked
        return val ?? false
      })
      return (
        <box
        ...
        >
          ...
          <text fg={checked() ? theme.text : theme.textMuted}>
            {checked() ? "■" : "□"}
          </text>
        </box>
      )
                      ...
                      return (
                        <box
                          ...
                          onMouseOver={() => {
                            ...
                            const idx = flat().findIndex((opt) => isDeepEqual(opt.value, option.value))
                            if (idx >= 0 && idx !== store.selected) moveTo(idx)
                          }}
                          onMouseUp={() => {
                            if (props.locked) return
                            option.onSelect?.(dialog)
                            ...
                            backgroundColor={
                              active()
                                ? actionFocused()
                                  ...
                                  : (option.bg ?? (props.compact ? RGBA.fromInts(50, 50, 55, 200) : theme.primary))
                                : RGBA.fromInts(0, 0, 0, 0)
                            }
                          >
                            <Show when={current()}>
                              <box position="absolute" left={1} flexShrink={0}>
                                <text fg={option.color ?? theme.primary}>●</text>
                              </box>
                            </Show>
                            ...
                            <Option
                              ...
                              color={option.color}
                            />
                            <Show when={option.rowActions?.length}>
                              <For each={option.rowActions}>
                                {(rowAction) => {
                                  const checked = () => rowAction.checked()
                                  const isFavorite = rowAction.icon === "★"
                                  return (
                                    <box
                                      flexDirection="row"
                                      alignItems="center"
                                      ...
                                      height={1}
                                      flexShrink={0}
                                      onMouseUp={(e: { stopPropagation(): void }) => { e.stopPropagation(); rowAction.onTrigger() }}
                                    >
                                      {isFavorite ? (
                                        <text fg={checked() ? theme.text : theme.textMuted}>
                                          {checked() ? "★" : "☆"}
  ...
  return (
    <>
      ...
      >
        ...
        <Show when={props.footer && !props.footerAfterDescription}>
          <span style={{ fg: props.active && !props.muted ? fg : theme.textMuted }}> {props.footer}</span>
        </Show>
