...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      OCTOCODE_CLI_NAME: `'${binary}'`,
      OCTOCODE_MODELS_DEV: modelsData,
      OCTOCODE_CHANNEL: `'${Script.channel}'`,
      OCTOCODE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@octocode-ai/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/farhanic017/octocode.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
