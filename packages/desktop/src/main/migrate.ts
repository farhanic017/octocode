...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.octocode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `octocode.settings.dat` is special: it maps to the `octocode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"octocode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // octocode.settings.dat → the electron settings store ("octocode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "octocode.global.dat").
  const storeName = filename === "octocode.settings.dat" ? "octocode.settings" : filename
