...
import { expectOctocodeVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveOctocode?: typeof resolveWslOctocode
...
export function createWslServersController(
  ...
  const setOctocodeCheck = (distro: string, check: WslOctocodeCheck) => {
    setState({
      octocodeChecks: {
        ...state.octocodeChecks,
        [distro]: check,
      },
    })
  }

  const checkOctocode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveOctocode ?? resolveWslOctocode)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return octocodeCheck(distro, resolved, version, appVersion)
  }

  const refreshOctocodeCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setOctocodeCheck(distro, await checkOctocode(distro, opts))
  }
  ...
  const refreshOctocodeCheckBackground = (id: string, distro: string) => {
    void checkOctocode(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setOctocodeCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl octocode check failed", { id, distro, message })
      })
  }

  const refreshOctocodeChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkOctocode(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setOctocodeCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl octocode check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshOctocodeCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async probeOctocode(name: string) {
      await runJob({ kind: "probe-octocode", distro: name, startedAt: Date.now() }, async (abort) => {
        await refreshOctocodeCheck(name, { signal: abort.signal })
      })
    },

    async installOctocode(name: string) {
      await runJob({ kind: "install-octocode", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslOctocode(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "OctoCode installation failed")
        }
        await refreshOctocodeCheck(name, { signal: abort.signal })
        expectOctocodeVersion(state.octocodeChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.octocodeChecks, distro) : {}),
      })
    },
...
function octocodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "octocode is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "octocode is installed but could not run",
    }
  }
  return {
    distro,
