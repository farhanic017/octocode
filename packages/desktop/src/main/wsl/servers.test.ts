...
let releaseOctocodeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectOctocodeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectOctocodeVersion("1.14.35", "1.16.2")).toThrow(
    "OctoCode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating OctoCode", () => {
  expect(
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.octocode/bin/octocode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, octocodeChecks: {} })
})
...
test("ignores stale background OctoCode checks after removing a WSL server", async () => {
  persistedServers = []
  releaseOctocodeResolve = undefined
  ...
  await waitFor(() => !!releaseOctocodeResolve)
  await controller.removeServer("wsl:Debian")
  releaseOctocodeResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().octocodeChecks).toEqual({})
})

test("ignores stale startup OctoCode checks after removing a WSL server", async () => {
...
function testControllerOptions() {
  return {
    ...
    resolveOctocode: async () => {
      await new Promise<void>((resolve) => {
        releaseOctocodeResolve = resolve
      })
      return "/home/me/.octocode/bin/octocode"
    },
  }
}
