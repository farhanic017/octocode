...
import { type WslCommandLine, resolveWslOctocode, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const octocode = await resolveWslOctocode(distro)
  if (!octocode) throw new Error(`OctoCode is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export OCTOCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export OCTOCODE_CLIENT=desktop",
    `export OCTOCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export OCTOCODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(octocode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
