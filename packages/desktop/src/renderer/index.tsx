...
import {
  ...
  useCommand,
  useWslServers,
} from "@octocode-ai/app"
import type { UpdaterState } from "@octocode-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@octocode-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.OCTOCODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "octocode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__OCTOCODE__ ??= {}
  const pending = window.__OCTOCODE__.deepLinks ?? []
  window.__OCTOCODE__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://octocode.ai/favicon-96x96-v3.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("octocode.global.dat").getItem("language")
