...
- Author package entrypoint is `@octocode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://octocode.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/octocode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@octocode-ai/plugin/tui`.
- `@octocode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running OctoCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.octocode` is absent, no check is performed (backward compatible).
...
- `octocode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `octocode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.octocode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.octocode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.octocode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.octocode` dir or `OCTOCODE_CONFIG_DIR`, OctoCode installs `@octocode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@octocode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus OctoCode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
OctoCode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, OctoCode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@octocode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"octo"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `octocode.default`.
...
- Local plugins persist installed themes under the local `.octocode/themes` area near the plugin config source.
...
- `--pure` / `OCTOCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.octocode/plugins/tui-smoke.tsx`
- Local vim plugin: `.octocode/plugins/tui-vim.tsx`
- Local smoke config: `.octocode/tui.json`
- Local smoke theme: `.octocode/plugins/smoke-theme.json`
