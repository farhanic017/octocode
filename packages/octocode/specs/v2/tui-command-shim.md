...
Delete `packages/octocode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/octocode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/octocode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/octocode`
- TUI plugin loader tests in `packages/octocode` if runtime plugin API wiring changed
