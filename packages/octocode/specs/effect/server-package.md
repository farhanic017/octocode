...
Practical reference for a future `packages/server` split after the octocode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/octocode`.
...
Until enough shared service code lives outside `packages/octocode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/octocode`

It should not import `packages/octocode` services while `packages/octocode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on octocode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/octocode` implementation details.
