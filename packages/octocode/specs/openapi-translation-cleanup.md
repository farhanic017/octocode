...
Trim `packages/octocode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/octocode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/octocode`.
- `[x]` `bun typecheck` from `packages/octocode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/octocode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/octocode`.
- `[x]` `bun dev generate > /tmp/octocode-openapi.json` from `packages/octocode`.
...
- Edit `packages/octocode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/octocode-openapi.json` from `packages/octocode`.
...
- Check source schemas in files like `packages/octocode/src/session/schema.ts`, `packages/octocode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/octocode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/octocode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
