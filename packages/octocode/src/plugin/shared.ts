...
export const DEPRECATED_PLUGIN_PACKAGES = ["octocode-openai-codex-auth", "octocode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, octocodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(octocodeVersion) || semver.major(octocodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.octocode
  if (typeof range !== "string") return
  if (!semver.satisfies(octocodeVersion, range)) {
    throw new Error(`Plugin requires octocode ${range} but running ${octocodeVersion}`)
  }
}
