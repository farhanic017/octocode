...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=octocode` lets xAI attribute octocode-originated
...
<html>
  <head>
    <title>octo code - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to octo code.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>octo code - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // behavior and crash the entire octocode process. Matches the silent-
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `octocode/${InstallationVersion}`)
