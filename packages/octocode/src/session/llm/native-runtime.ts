...
import type { LLMClientShape } from "@octocode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("octo"))
    return { type: "unsupported", reason: "provider is not openai, octocode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @octocode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains octocode-owned. The native runtime only adapts
      // the @octocode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
