...
import { Slug } from "@octocode-ai/core/util/slug"
...
import type { ProviderMetadata, Usage } from "@octocode-ai/llm"
...
import { PartTable, SessionTable } from "@octocode-ai/core/session/sql"
...
import { NonNegativeInt, optionalOmitUndefined } from "@octocode-ai/core/schema"
...
export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".octo", "plans")
...
export class Service extends Context.Service<Service, Interface>()("@octocode/Session") {}
    ...
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      if (!title) {
        if (input.parentID) {
          ...
          // Try dynamic naming, fall back to timestamp
          const namingModule = yield* Effect.tryPromise({
            try: () => import("./naming"),
            catch: () => null as typeof import("./naming") | null,
          }).pipe(Effect.catch(() => Effect.succeed(null as typeof import("./naming") | null)))

          if (namingModule?.generateSessionName) {
            title = yield* Effect.tryPromise({
              try: () => namingModule.generateSessionName(input.directory, ctx.worktree),
              catch: () => parentTitlePrefix + new Date().toISOString(),
            }).pipe(Effect.catch(() => Effect.succeed(parentTitlePrefix + new Date().toISOString())))
          } else {
            title = parentTitlePrefix + new Date().toISOString()
          }
        }
      }
