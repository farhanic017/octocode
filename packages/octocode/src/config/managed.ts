...
const MANAGED_PLIST_DOMAIN = "ai.octocode.managed"

// Keys injected by macOS/MDM into the managed plist that are not octo code config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/octocode"
    case "win32":
...
export function managedConfigDir() {
  return process.env.OCTOCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
