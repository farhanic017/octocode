...
const TUI_SCHEMA_URL = "https://octocode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from octo/octocode config files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const configFiles = await octocodeFiles(input)
...
async function octocodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "octo"),
    ...(await Filesystem.findUp(["octo.json", "octo.jsonc", "octocode.json", "octocode.jsonc"], input.cwd, undefined, {
      rootFirst: true,
    })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "octo"))
  }
  if (Flag.OCTOCODE_CONFIG) files.push(Flag.OCTOCODE_CONFIG)
