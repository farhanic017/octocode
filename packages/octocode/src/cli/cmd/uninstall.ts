...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall octo code and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall octo code")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g octocode-ai",
      pnpm: "pnpm uninstall -g octocode-ai",
      bun: "bun remove -g octocode-ai",
      yarn: "yarn global remove octocode-ai",
      brew: "brew uninstall octocode",
      choco: "choco uninstall octocode",
      scoop: "scoop uninstall octocode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      ...
      bun: ["bun", "remove", "-g", "octocode-ai"],
      yarn: ["yarn", "global", "remove", "octocode-ai"],
      brew: ["brew", "uninstall", "octo"],
      choco: ["choco", "uninstall", "octo"],
      scoop: ["scoop", "uninstall", "octo"],
    }

    const cmd = cmds[method]
    if (cmd) {
      spinner.start(`Running ${cmd.join(" ")}...`)
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "octo", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using octo code!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# octocode") || content.includes(".octocode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".octocode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".octocode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".octocode"))
    ) {
      continue
    }
