...
import { OctocodeKeymapProvider } from "@octocode-ai/tui/keymap"
...
export class RunFooter implements FooterApi {
  ...
  constructor(
    ...
    void render(
      () =>
        createComponent(OctocodeKeymapProvider, {
    ...
    this.flushing = this.flushing
      .then(() =>
        withRunSpan(
          "RunFooter.flush",
          {
            "octocode.batch.commits": batch.length,
            "octocode.footer.phase": phase,
            "session.id": this.options.sessionID() || undefined,
          },
          async () => {
