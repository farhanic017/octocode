...
import { createFrames } from "@octocode-ai/tui/ui/spinner"
...
export function RunFooterSubagentBody(props: {
  ...
  const spinnerFrames = createMemo(() => createFrames())
          ...
          {(current) => (
            <box width="100%" flexDirection="row" gap={1} paddingBottom={1} flexShrink={0}>
              {current().status === "running" ? (
                <box flexShrink={0}>
                  <spinner frames={spinnerFrames()} interval={80} color={statusColor(footer(), current().status)} />
                </box>
              ) : (
