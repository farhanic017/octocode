...
import { createOctocodeClient } from "@octocode-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput, deps: RunRuntimeDeps = {}): Promise<void> {
  return withRunSpan(
    ...
    {
      "octocode.mode": input.resolveSession ? "local" : "attach",
      "octocode.initial_input": !!input.initialInput,
      "octocode.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "octocode.directory": ctx.directory,
        "octocode.resume": ctx.resume === true,
        "octocode.agent.name": state.agent,
        "octocode.model.provider": state.model?.providerID,
        "octocode.model.id": state.model?.modelID,
        "octocode.model.variant": state.activeVariant,
      ...
      const shell = await (deps.createRuntimeLifecycle ?? createRuntimeLifecycle)({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "octocode.model.provider": model.providerID,
            "octocode.model.id": model.modelID,
            "octocode.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "octocode.model.id": state.model?.modelID,
                "octocode.model.variant": state.activeVariant,
                "octocode.prompt.chars": prompt.text.length,
                "octocode.prompt.parts": prompt.parts.length,
                "octocode.prompt.include_files": includeFiles,
                "octocode.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "octocode.directory": input.directory,
      "octocode.initial_input": !!input.initialInput,
      "octocode.demo": input.demo,
    },
    async () => {
