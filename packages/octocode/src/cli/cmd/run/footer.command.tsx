...
function PanelShell(props: {
  ...
  const useFullWidth = props.fullWidth !== false
  const content = (
    <>
  ...
  return (
    <box id={props.id} width={useFullWidth ? "100%" : "auto"} flexDirection="column" border={false} backgroundColor="transparent" flexShrink={0}>
      {minimal() ? (
...
export function RunModelSelectBody(props: {
  ...
  const entries = createMemo<ModelEntry[]>(() =>
    (props.providers() ?? [])
        ...
        Object.entries(provider.models)
          ...
          .map(([modelID, model]) => {
            ...
            const footer = current
              ? "current"
              : model.cost?.input === 0 && provider.id === "octo"
                ? "Free"
                : title !== modelID
                  ? modelID
                  : undefined
      ...
      .sort((a, b) => {
        const provider = Number(a.providerID !== "octo") - Number(b.providerID !== "octo")
        if (provider !== 0) {
          return provider
        }
