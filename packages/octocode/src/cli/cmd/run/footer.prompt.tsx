...
import { normalizePromptContent } from "@octocode-ai/tui/editor"
...
import { OCTOCODE_BASE_MODE, useBindings } from "@octocode-ai/tui/keymap"
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  const slashOptions = createMemo<SlashOption[]>(() => {
    const builtins = [
      ...
      { kind: "slash", name: "exit", display: "/exit", description: "close octo code" } satisfies SlashOption,
    ]
  ...
  const syncRows = () => {
    ...
    try {
      const lc = area.lineCount ?? 1
      const vlc = area.virtualLineCount ?? 1
      input.onRows(clamp(Math.max(lc, vlc)) + popup())
    } catch {}
  }
  ...
  const syncParts = () => {
    ...
    try {
      ...
      const extmarks = area.extmarks.getAllForTypeId(type)
      if (!extmarks) return
      for (const item of extmarks) {
        if (!item || item.start == null || item.end == null) continue
        ...
        const plainText = area.plainText
        if (!plainText || item.start >= plainText.length || item.end > plainText.length) continue
        const text = plainText.slice(item.start, item.end)
        const prev =
          part.type === "agent"
  ...
  const restoreParts = (value: RunPromptPart[]) => {
    ...
    try {
      ...
      parts.forEach((item, idx) => {
        ...
        if (plainText && end > plainText.length) return
