...
import { openEditor } from "@octocode-ai/tui/editor"
import { registerOctocodeKeymap } from "@octocode-ai/tui/keymap"
...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "octocode.agent.name": input.agent,
      "octocode.directory": input.directory,
      "octocode.first": input.first,
      "octocode.model.provider": input.model?.providerID,
      "octocode.model.id": input.model?.modelID,
      "octocode.model.variant": input.variant,
    ...
    async () => {
      ...
      try {
        ...
        unregisterKeymap = registerOctocodeKeymap(keymap, renderer, input.tuiConfig)
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "octocode.show_exit": next.showExit,
