...
import type { Hooks } from "@octocode-ai/plugin"
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.url) {
      ...
      const wellknown = (yield* cliTry(`Failed to load auth provider metadata from ${url}: `, () =>
        fetch(`${url}/.well-known/octocode`).then((x) => x.json()),
      )) as {
    ...
    if (provider === "octo") {
      yield* Prompt.log.info("Create an api key in the octo code model portal")
    }
