...
import { ConfigMCPV1 } from "@octocode-ai/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: octo mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in octo.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer Octo names, check project config subdirectories too)
  const candidates = [
    path.join(baseDir, "octo.json"),
    path.join(baseDir, "octo.jsonc"),
    path.join(baseDir, "octocode.json"),
    path.join(baseDir, "octocode.jsonc"),
  ]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".octo", "octo.json"),
      path.join(baseDir, ".octo", "octo.jsonc"),
      path.join(baseDir, ".octocode", "octocode.json"),
      path.join(baseDir, ".octocode", "octocode.jsonc"),
    )
  }
  ...
  // Default to octo.json if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., octo x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "octocode-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
