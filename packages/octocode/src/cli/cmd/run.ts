import type { PermissionV1 } from "@octocode-ai/core/v1/permission"
// CLI entry point for `octo run`.
//
// Handles three modes:
...
//      octo code server and runs interactive mode against it.
//
...
import { createOctocodeClient, type OctocodeClient, type ToolPart } from "@octocode-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<OctocodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run octo code with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running octo code server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OCTOCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OCTOCODE_SERVER_USERNAME or 'octo')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: OctocodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: OctocodeClient, sessionID: string) {
      ...
      async function current(sdk: OctocodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: OctocodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: OctocodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: OctocodeClient) {
      ...
      const sdk = createOctocodeClient({
        baseUrl: "http://octocode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
