...
import type { Message, OctocodeClient, SessionMessageResponse } from "@octocode-ai/sdk/v2"
...
export const AuthMethodID = "octocode-login"
...
export class Service extends Context.Service<Service, Interface>()("@octocode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `octo auth login` in the terminal",
      name: "Login with octo code",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: OctocodeClient) {
...
function makeUsageService(sdk: OctocodeClient) {
...
async function loadDirectorySnapshot(sdk: OctocodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, octocode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const octocodeProvider = providers[ProviderV2.ID.make("octo")]
  const octocodeModel = octocodeProvider ? Provider.sort(Object.values(octocodeProvider.models))[0] : undefined
  if (octocodeProvider && octocodeModel) return { providerID: octocodeProvider.id, modelID: octocodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "octo code service failure", service })
}
