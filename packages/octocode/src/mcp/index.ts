...
export class Service extends Context.Service<Service, Interface>()("@octocode/MCP") {}
    ...
    const connectTransport = (transport: Transport, timeout: number) =>
      Effect.acquireUseRelease(
        ...
        (t) =>
          Effect.tryPromise({
            try: () => {
              const client = new Client({ name: "octo", version: InstallationVersion })
            ...
            if (isAuthError) {
              ...
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: octo mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
