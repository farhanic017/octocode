...
describe("tool.read external_directory permission", () => {
  ...
  references.live("does not ask for external_directory permission when reading configured references", () =>
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const cache = path.join(Global.Path.repos, "github.com", "octocode-read-reference", "repo")
      ...
      const remoteDir = path.join(remoteRoot, "octocode-read-reference")
      ...
      const dir = yield* tmpdirScoped({
        ...
        config: {
          reference: {
            docs: "octocode-read-reference/repo",
          },
        },
      })
