  ...
  it.instance("does not ask for external_directory when alias path is allowed", () =>
    Effect.gen(function* () {
      ...
      const tmp = yield* Effect.acquireRelease(
        Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "octocode-grep-alias-"))),
  ...
  references.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const cache = path.join(Global.Path.repos, "github.com", "octocode-grep-reference", "repo")
        ...
        const remoteDir = path.join(remoteRoot, "octocode-grep-reference")
      ...
      }),
    {
      config: {
        reference: {
          docs: "octocode-grep-reference/repo",
        },
      },
    },
  )
