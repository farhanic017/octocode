...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "octocode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "octocode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "octocode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "octocode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["octocode.json", "octocode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "octocode.json"),
        path.join(project, "octocode.jsonc"),
      ])
    })
  })
