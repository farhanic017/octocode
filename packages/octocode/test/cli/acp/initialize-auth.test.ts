...
describe("octocode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ octocode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ octocode }))
        ...
        expect(initialized.agentInfo?.name).toBe("OctoCode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ octocode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ octocode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("octocode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "octocode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.OCTOCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ octocode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("octocode-login")
