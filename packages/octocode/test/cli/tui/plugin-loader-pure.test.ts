...
`,
  ...
  const pure = process.env.OCTOCODE_PURE
  const meta = process.env.OCTOCODE_PLUGIN_META_FILE
  process.env.OCTOCODE_PURE = "1"
  process.env.OCTOCODE_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.OCTOCODE_PURE
    } else {
      process.env.OCTOCODE_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.OCTOCODE_PLUGIN_META_FILE
    } else {
      process.env.OCTOCODE_PLUGIN_META_FILE = meta
    }
  }
})
