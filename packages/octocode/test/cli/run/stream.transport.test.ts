...
import { OctocodeClient, type GlobalEvent } from "@octocode-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<OctocodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<OctocodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<OctocodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<OctocodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<OctocodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://octocode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: OctocodeClient["event"]["subscribe"]
    globalEvent?: OctocodeClient["global"]["event"]
    promptAsync?: OctocodeClient["session"]["promptAsync"]
    status?: OctocodeClient["session"]["status"]
    messages?: OctocodeClient["session"]["messages"]
    children?: OctocodeClient["session"]["children"]
    permissions?: OctocodeClient["permission"]["list"]
    questions?: OctocodeClient["question"]["list"]
  } = {},
) {
  const client = new OctocodeClient()

  const subscribe: OctocodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: OctocodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: OctocodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: OctocodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: OctocodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: OctocodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: OctocodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: OctocodeClient["question"]["list"] = input.questions ?? (() => ok([]))
