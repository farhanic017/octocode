...
describe("octocode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // OCTOCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ octocode }) =>
      Effect.gen(function* () {
        const r = yield* octocode.spawn(["mcp", "list"])
        octocode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ octocode }) =>
      Effect.gen(function* () {
        const r = yield* octocode.spawn(["providers", "list"])
        octocode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ octocode }) =>
      Effect.gen(function* () {
        const r = yield* octocode.spawn(["models"])
        octocode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ octocode }) =>
      Effect.gen(function* () {
        const r = yield* octocode.spawn(["agent", "list"])
        octocode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh OCTOCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ octocode }) =>
      Effect.gen(function* () {
        const r = yield* octocode.spawn(["session", "list"])
        octocode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ octocode }) =>
      Effect.gen(function* () {
        const r = yield* octocode.spawn(["stats"])
        octocode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under OCTOCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ octocode }) =>
      Effect.gen(function* () {
        const r = yield* octocode.spawn(["db", "path"])
        octocode.expectExit(r, 0, "db path")
