...
import { EventSequenceTable } from "@octocode-ai/core/event/sql"
...
import { WorkspaceTable } from "@octocode-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  OCTOCODE_AUTH_CONTENT: process.env.OCTOCODE_AUTH_CONTENT,
  OCTOCODE_EXPERIMENTAL_WORKSPACES: process.env.OCTOCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.OCTOCODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@octocode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.OCTOCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=octocode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.OCTOCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.OCTOCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.OCTOCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=octocode-test")
