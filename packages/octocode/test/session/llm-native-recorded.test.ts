...
import { HttpRecorder } from "@octocode-ai/http-recorder"
import { HttpRecorderInternal } from "@octocode-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@octocode-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@octocode-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.octocode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.OCTOCODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["octo", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OCTOCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("OCTOCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["octo", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "octocode-proxy",
    name: "OctoCode proxy",
    providerID: ProviderV2.ID.octocode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["octo", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.OCTOCODE_RECORD_CONSOLE_TOKEN && process.env.OCTOCODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.octocode,
        name: "OctoCode Zen",
        env: ["OCTOCODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.OCTOCODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.OCTOCODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.OCTOCODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["octo", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OCTOCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("OCTOCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("OCTOCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set OCTOCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the octocode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://octocode.ai/config.json", ...scenario.config(model) }),
    ),
  )
