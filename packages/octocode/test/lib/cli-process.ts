// Subprocess test harness for the octocode CLI. Spawns the real binary against
...
//
// Configuration flows through octocode's built-in test affordances:
//   - OCTOCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - OCTOCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - OCTOCODE_DISABLE_PROJECT_CONFIG : skip walking up for octocode.json
//   - OCTOCODE_PURE                : skip external plugin discovery + install
//   - OCTOCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `octocode.run` is fully wired. The shape supports adding more
// builders (`octocode.serve(opts)`, `octocode.acp(opts)`, `octocode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OctocodeCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@octocode-ai/core/fs-util"
import { AppProcess } from "@octocode-ai/core/process"
...
const octocodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(octocodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    OCTOCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    OCTOCODE_CONFIG_CONTENT: configJson,
    OCTOCODE_DISABLE_PROJECT_CONFIG: "1",
    OCTOCODE_PURE: "1",
    OCTOCODE_DISABLE_AUTOUPDATE: "1",
    OCTOCODE_DISABLE_AUTOCOMPACT: "1",
    OCTOCODE_DISABLE_MODELS_FETCH: "1",
    OCTOCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `octocode run`. New flags should
...
// `octocode serve` is a long-lived process — it never exits on its own.
...
// `octocode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OctocodeCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `octocode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `octocode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("octocode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("octocode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "octocode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `octocode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("octocode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const octocode: OctocodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, octocode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `octocode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "octo") {
...
// every call site. Use this for any test that needs the octocode CLI fixture.
//
...
// resources (e.g. `octocode.serve`) without an extra `Effect.scoped` wrapper —
