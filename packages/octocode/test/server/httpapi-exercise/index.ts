 ...
 *
 * The script intentionally isolates `OCTOCODE_DB` before importing modules that touch
...
void (await import("@octocode-ai/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "octocode.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "octocode.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-octocode-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-octocode-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "octo", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
