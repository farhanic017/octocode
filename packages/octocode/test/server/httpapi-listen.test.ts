...
const original = {
  ...
  envPassword: process.env.OCTOCODE_SERVER_PASSWORD,
  envUsername: process.env.OCTOCODE_SERVER_USERNAME,
}
const auth = { username: "octo", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.OCTOCODE_SERVER_PASSWORD
  else process.env.OCTOCODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.OCTOCODE_SERVER_USERNAME
  else process.env.OCTOCODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.OCTOCODE_SERVER_PASSWORD = auth.password
  Flag.OCTOCODE_SERVER_USERNAME = auth.username
  process.env.OCTOCODE_SERVER_PASSWORD = auth.password
  process.env.OCTOCODE_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.OCTOCODE_SERVER_USERNAME = auth.username
  delete process.env.OCTOCODE_SERVER_PASSWORD
  process.env.OCTOCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-octocode-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-octocode-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-octocode-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-octocode-ticket": "1" },
      })
