...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-octocode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-octocode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://octocode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-octocode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-octocode-workspace")
