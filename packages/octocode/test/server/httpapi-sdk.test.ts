...
type Sdk = ReturnType<typeof createOctocodeClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.OCTOCODE_SERVER_PASSWORD = input?.password
      Flag.OCTOCODE_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".octocode", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-octocode-directory")).toBe(false)
        expect(request!.headers.has("x-octocode-workspace")).toBe(false)
      }),
    ),
  )
