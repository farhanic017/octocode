...
it.instance("plan agent denies edits except .octocode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".octocode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".octocode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OCTOCODE_TEST_HOME
      process.env.OCTOCODE_TEST_HOME = test.directory
