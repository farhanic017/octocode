...
Replace runtime Bun shell template-tag usage in `packages/octocode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/octocode/src` is removed except:
