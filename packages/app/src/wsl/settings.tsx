...
import { Tag } from "@octocode-ai/ui/v2/badge-v2"
import { ButtonV2 } from "@octocode-ai/ui/v2/button-v2"
import { Dialog } from "@octocode-ai/ui/v2/dialog-v2"
...
import { wslOctocodeAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.octocodeChecks[item.config.distro]
          const octocodeAction = () => wslOctocodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-octocode" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={octocodeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installOctocode(item.config.distro))}
                    >
