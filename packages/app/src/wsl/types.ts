...
export type WslJob =
  ...
  | { kind: "probe-octocode"; distro: string; startedAt: number }
  | { kind: "install-octocode"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeOctocode(name: string): Promise<void>
  installOctocode(name: string): Promise<void>
