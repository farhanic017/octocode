import { Button } from "@octocode-ai/ui/button"
import { useDialog } from "@octocode-ai/ui/context/dialog"
import { Spinner } from "@octocode-ai/ui/spinner"
import { showToast } from "@octocode-ai/ui/toast"
import { createEffect, createMemo, For, Match, onCleanup, Show, Switch } from "solid-js"
import { createStore } from "solid-js/store"
import { useLanguage } from "@/context/language"
import { usePlatform } from "@/context/platform"
import { useWslServers } from "./context"
import { enterWslOctocodeStep } from "./settings-model"

type WslServerStep = "wsl" | "distro" | "octo"

const STEPS: WslServerStep[] = ["wsl", "distro", "octo"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const octocodeCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.octocodeChecks[distro] ?? null
  })
  ...
  const octocodeReady = createMemo(() => {
    const check = octocodeCheck()
  ...
  const installingOctocode = createMemo(() => {
    const job = current()?.job
    return job?.kind === "install-octocode" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && octocodeReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-octocode"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.octocodeChecks[distro]) {
      return { key: `probe-octocode:${distro}`, run: () => api.probeOctocode(distro) }
    }
    return null
  })
  ...
  const octocodeMessage = createMemo(() => {
    const state = current()
    if (!state) return language.t("wsl.onboarding.checkingOctocode")
    const distro = selectedDistro()
    if (state.job?.kind === "install-octocode") {
      return distro
        ? language.t("wsl.onboarding.updatingOctocodeIn", { distro })
        : language.t("wsl.onboarding.updatingOctocode")
    }
    if (state.job?.kind === "probe-octocode") {
      return distro
        ? language.t("wsl.onboarding.checkingOctocodeIn", { distro })
        : language.t("wsl.onboarding.checkingOctocode")
    }
    if (octocodeCheck()?.error) return octocodeCheck()!.error
    if (octocodeCheck()?.matchesDesktop === false) {
      return distro
        ? language.t("wsl.onboarding.updateOctocodeIn", { distro })
        : language.t("wsl.onboarding.updateOctocode")
    }
    if (octocodeReady()) {
      return distro
        ? language.t("wsl.onboarding.octocodeReadyIn", { distro })
        : language.t("wsl.onboarding.octocodeReady")
    }
    return distro
      ? language.t("wsl.onboarding.installOctocodeIn", { distro })
  ...
  const openOctocodeStep = () => {
    const distro = selectedDistro()
    if (!distro) return
    void run(() => enterWslOctocodeStep(distro, api.probeOctocode, (step) => setStore("step", step)))
  }
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.octocode"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : octocodeCheck()?.matchesDesktop === false
                  ? "warning"
                  : octocodeReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "octo"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.octocode")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.probeOctocode(distro))}
                      >
                        {language.t("wsl.onboarding.refresh")}
                      </Button>
                    </Show>
                    <Show when={!octocodeReady() || octocodeCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.installOctocode(distro))}
                      >
                        <Show when={installingOctocode()}>
                          <Spinner class="size-4 shrink-0" />
                        </Show>
                        {octocodeCheck()?.resolvedPath
                          ? language.t("wsl.onboarding.updateOctocode")
                          : language.t("wsl.onboarding.installOctocode")}
                      </Button>
                    </Show>
                  </div>
                </div>
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{octocodeMessage()}</div>
                <Show when={octocodeCheck()?.matchesDesktop === false ? octocodeCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "octo" && allReady() && selectedDistro()}>
