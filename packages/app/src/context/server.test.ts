  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://octocode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://octocode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://octocode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://octocode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://octocode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://octocode.example.com"),
      ),
    ).toEqual({
