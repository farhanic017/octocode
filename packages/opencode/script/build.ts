...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/octocode`,
      execArgv: [`--user-agent=octocode/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/octocode`
    ...
    JSON.stringify(
      {
        ...
        keywords: ["ai", "coding", "agent", "cli", "octocode"],
        os: [item.os],
        cpu: [item.arch],
      },
      null,
