...
import { AppFileSystem } from "@octo-ai/shared/filesystem"
import { InstanceState } from "@/effect"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { EffectFlock } from "@octo-ai/shared/util/effect-flock"
...
const InfoSchema = Schema.Struct({
  ...
  compaction: Schema.optional(
    Schema.Struct({
      ...
      mode: Schema.optional(Schema.Literals(["llm", "token-budget", "auto"])).annotate({
        description:
          "Compaction mode: 'llm' uses LLM summarization (default), 'token-budget' skips LLM and keeps only recent turns, 'auto' uses token-budget when context >90% full",
      }),
      ...
      trim_threshold: Schema.optional(NonNegativeInt).annotate({
        description: "Character length threshold for soft-trimming function call outputs before compaction (default: 4096)",
      }),
    }),
  ),
...
function globalConfigFile() {
  const candidates = ["octocode.jsonc", "octocode.json", "config.json"].map((file) =>
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "octocode.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "octocode.jsonc"))),
      )
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.MIMOCODE_DISABLE_PROJECT_CONFIG) {
          // Try octocode.json first, fall back to mimocode.json
          const octoFiles = yield* ConfigPaths.files("octocode", ctx.directory, ctx.worktree).pipe(Effect.orDie)
          const mimoFiles = yield* ConfigPaths.files("mimocode", ctx.directory, ctx.worktree).pipe(Effect.orDie)
          const allFiles = [...octoFiles, ...mimoFiles.filter((f) => !octoFiles.includes(f.replace("mimocode", "octocode")))]
        ...
        // Load Claude Code commands first so .octocode commands override on name collision.
        for (const dir of yield* ConfigPaths.claudeCommandDirectories(ctx.directory, ctx.worktree)) {
          result.command = mergeDeep(result.command ?? {}, yield* Effect.promise(() => ConfigCommand.load(dir)))
        }

        for (const dir of directories) {
          if (dir.endsWith(".octocode") || dir === Flag.MIMOCODE_CONFIG_DIR) {
            for (const file of ["octocode.json", "octocode.jsonc"]) {
