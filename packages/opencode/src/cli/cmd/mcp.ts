    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in octocode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .octocode/ subdirectory too)
  const candidates = [path.join(baseDir, "octocode.json"), path.join(baseDir, "octocode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".octocode", "octocode.json"), path.join(baseDir, ".octocode", "octocode.jsonc"))
  }
  ...
  // Default to octocode.json if none exist
  return candidates[0]
}
