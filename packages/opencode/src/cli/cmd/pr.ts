...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run octocode",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found octocode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["octo", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting octocode...")
        UI.println()

        const octoArgs = sessionId ? ["-s", sessionId] : []
        const octoProcess = Process.spawn(["octo", ...octoArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await octoProcess.exited
        if (code !== 0) throw new Error(`octo exited with code ${code}`)
      },
    })
  },
})
