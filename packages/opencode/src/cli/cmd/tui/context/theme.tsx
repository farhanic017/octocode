...
import { Glob } from "@octo-ai/shared/util/glob"
...
import octocode from "./theme/octocode.json" with { type: "json" }
import octocodeDark from "./theme/octocode-dark.json" with { type: "json" }
import octocodeLight from "./theme/octocode-light.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@octo-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "octocode-dark")
        draft.active = typeof active === "string" ? active : "octocode-dark"
        draft.ready = false
      }),
    )
