...
import { useTheme, selectedForeground } from "@/cli/cmd/tui/context/theme"
...
export function DialogSelect<T>(props: DialogSelectProps<T>) {
  ...
  const height = createMemo(() => props.maxHeight ?? Math.min(rows(), Math.floor(dimensions().height * 0.6) - 2))
          ...
          <box flexDirection="row" gap={1} paddingTop={1} justifyContent="center">
            <For each={props.filterButtons}>
              {(btn) => {
                ...
                const fg = () => btn.active ? pink : muted
                return (
                  <box
                    width={13}
                    paddingLeft={1}
