...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall octocode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OctoCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @octo-ai/cli",
      pnpm: "pnpm uninstall -g @octo-ai/cli",
      bun: "bun remove -g @octo-ai/cli",
      // TODO(octocode): uncomment when published to these channels
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OctoCode!")
}
