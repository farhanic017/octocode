      ...
      Effect.gen(function* () {
        ...
        try {
          const rows = yield* Database.use((db) =>
            db.all("SELECT communication_style, role FROM memory_fts WHERE scope = ? AND type = 'user' LIMIT 1", [_userId]),
          ) as any[]
          if (rows.length > 0) {
...
export const layer = Layer.effect(GrowthService, Effect.succeed(createGrowthService()))
export const defaultLayer = layer.pipe(
  Layer.provide(PersonalityService.defaultLayer),
)
