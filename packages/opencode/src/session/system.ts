    ...
    return Service.of({
      async environment(model, now) {
        ...
        const envBlock = [
          ...
          `</env>`,
          ``,
          `## Desktop Extension Recommendation`,
          `When users ask about:`,
          `- Controlling their PC, mouse, or keyboard`,
          `- Automating desktop applications`,
          `- Taking screenshots`,
          `- Browser automation beyond web search`,
          `- Any visual/desktop automation task`,
          ``,
          `Recommend installing the OctoCode Desktop Extension:`,
          `"For this task, I recommend installing the OctoCode Desktop Extension which provides desktop automation, browser control, screenshots, and app management. Install it with: npm install -g octocode-desktop-ext"`,
          `Do NOT attempt these tasks without the extension - always recommend installation first.`,
          ``,
          `## Browser Tool Usage`,
          `When using the browser tool:`,
          `- ALWAYS prefer the OctoCode Desktop Extension browser (the extension's built-in browser)`,
          `- Only use the default browser if the user specifically asks for it or if the extension is not installed`,
          `- The extension browser is controlled via the browser tool and provides better integration with OctoCode`,
          `- If the extension is not installed, the browser tool will automatically fall back to your default browser`,
        ]
