...
// Process metadata for telemetry and identification
export const MIMOCODE_PROCESS_ROLE = process.env.MIMOCODE_PROCESS_ROLE || "main"
export const MIMOCODE_RUN_ID = process.env.MIMOCODE_RUN_ID || crypto.randomUUID()

export function ensureRunID(): string {
  if (!process.env.MIMOCODE_RUN_ID) {
    process.env.MIMOCODE_RUN_ID = MIMOCODE_RUN_ID
  }
  return process.env.MIMOCODE_RUN_ID
}

export function ensureProcessMetadata(role: string): { processRole: string; runID: string } {
  process.env.MIMOCODE_PROCESS_ROLE = role
  const runID = ensureRunID()
  return { processRole: role, runID }
}

export function sanitizedProcessEnv(extra?: Record<string, string>): Record<string, string> {
  const env: Record<string, string> = { ...process.env as Record<string, string>, ...extra }
  // Remove sensitive variables
  delete env.MIMOCODE_SERVER_PASSWORD
  delete env.MIMOCODE_AUTH_TOKEN
  return env
}
