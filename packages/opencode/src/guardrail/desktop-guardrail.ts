...
import { GuardrailService, GuardrailCheck } from "./guardrail"
...
export class DesktopGuardrailService {
  ...
  constructor() {
    this.guardrail = new GuardrailService({
      forbiddenPatterns: [
  ...
  checkDesktopAction(toolId: string, params: Record<string, unknown>): GuardrailCheck {
    const general = this.guardrail.check(toolId, params)
    ...
    if (toolId === "desktop_control" && params.action === "type") {
      ...
      for (const pattern of CREDENTIAL_PATTERNS) {
        if (pattern.test(text)) {
          return { level: "restricted", blocked: true, needsConfirmation: false, reason: "Credential text detected in typed content" }
        }
      }
    }
