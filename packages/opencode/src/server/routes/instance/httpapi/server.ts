...
const Headers = Schema.Struct({
  ...
  "x-octocode-directory": Schema.optional(Schema.String),
})
  ...
  Authorization.of({
    basic: (effect, { credential }) =>
      Effect.gen(function* () {
        ...
        const user = Flag.MIMOCODE_SERVER_USERNAME ?? "octocode"
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-octocode-directory"] || process.cwd()
