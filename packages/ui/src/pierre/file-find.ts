...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("octocode-find")
  api.delete("octocode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("octocode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("octocode-find", new Highlight(...rest))
    return true
  }
