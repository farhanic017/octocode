...
const skip = new Set([".git", ".octocode", ".turbo", "dist", "node_modules"])
const keys = ["@octotui/core", "@octotui/keymap", "@octotui/solid"] as const
...
async function fixKnownLockfileIssues() {
  ...
  if (stale.some((item) => !item.entry.startsWith("opentui-spinner/@octotui/"))) return []

  const removed = txt
    .split("\n")
    .map((line) => line.match(/^    "(opentui-spinner\/@octotui\/[^\"]+)": /)?.[1])
  ...
  await Bun.write(
    ...
    txt
      .split("\n")
      .filter((line) => !line.match(/^    "opentui-spinner\/@octotui\//))
      .join("\n"),
  )
  return removed
}
...
function findStaleLockfileEntriesInText(txt: string) {
  return Array.from(txt.matchAll(/^    "([^"]+)": \["(@octotui\/(?:core(?:-[^@"]+)?|keymap|solid))@([^"]+)"/gm))
    .map((match) => ({
      entry: match[1]!,
      pkg: match[2]!,
      version: match[3]!,
