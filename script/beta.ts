...
const model = "octocode/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/octocode")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with octocode...`)
  ...
  try {
    await $`octocode run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  octocode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with octocode")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with octocode...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/octocode`.",
