...
import { createOctocode } from "@octocode-ai/sdk"
...
`)
  ...
  const octocode = await createOctocode({ port: 0 })
  ...
  try {
    ...
    const session = await octocode.client.session.create()
    const result = await octocode.client.session
      .prompt({
