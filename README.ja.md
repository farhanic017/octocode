...
# パッケージマネージャー
npm i -g octocode-ai@latest        # bun/pnpm/yarn でもOK
scoop install octocode             # Windows
choco install octocode             # Windows
brew install farhanic017/tap/octocode # macOS と Linux（推奨。常に最新）
brew install octocode              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S octocode            # Arch Linux (Stable)
paru -S octocode-bin               # Arch Linux (Latest from AUR)
mise use -g octocode               # どのOSでも
nix run nixpkgs#octocode           # または github:farhanic017/octocode で最新 dev ブランチ
```
...
OctoCode はデスクトップアプリとしても利用できます。[releases page](https://github.com/farhanic017/octocode/releases) から直接ダウンロードするか、[octocode.ai/download](https://octocode.ai/download) を利用してください。
...
1. `$OCTOCODE_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.octocode/bin` - デフォルトのフォールバック
...
OctoCode には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://octocode.ai/docs/agents) の詳細はこちら。

### ドキュメント

OctoCode の設定については [**ドキュメント**](https://octocode.ai/docs) を参照してください。

### コントリビュート

OctoCode に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### OctoCode の上に構築する

OctoCode に関連するプロジェクトで、名前に "octo"（例: "octocode-dashboard" や "octocode-mobile"）を含める場合は、そのプロジェクトが OctoCode チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/octocode) | [X.com](https://x.com/octocode)
