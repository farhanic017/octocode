...
Both `action` and `resource` use octocode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.octocode` files is intentionally deferred until `.octocode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether octocode permits an operation through its managed execution paths.
