...
The octocode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@octocode-ai/llm` tool loop after replacing its
...
- plugins get global "octo" instance like in that post i showed
- octocode instance has stuff like `octocode.session.prompt()` or
  `octocode.tool.register({...})`
...
- decide whether to preserve deprecated `@octocode-ai/llm` orchestration exports
