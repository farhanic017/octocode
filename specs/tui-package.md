...
Move the canonical OctoCode terminal application from
`packages/octocode/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/octocode ---\
                      > @octocode-ai/tui -> @octocode-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/octocode`,
`packages/cli`, or `@octocode-ai/core`.

The SDK is the TUI's OctoCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/octocode`.
- Do not replace private `packages/octocode` imports with `@octocode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@octocode-ai/tui`.

## Ownership Boundary

### `@octocode-ai/tui` Owns
...
- OctoCode domain data displayed by the TUI
...
```text
packages/octocode/src/cli/cmd/tui
```

Its private dependency on `packages/octocode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/octocode/src/*`.
...
Status: Completed. The private `@octocode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@octocode-ai/tui`.
...
- The package has no dependency on `octo`, `@octocode-ai/cli`, or
  `@octocode-ai/core`.
...
primitives now live in `@octocode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@octocode-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads OctoCode globals or process state;
...
Replace process-global OctoCode state with resolved TUI inputs.
...
`@octocode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/octocode` initially.
...
canonical in `@octocode-ai/tui`. Configured references resolve through the new
...
adapters rather than OctoCode domain implementations.

Make the SDK the only OctoCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports OctoCode data and operations only from
  `@octocode-ai/sdk`.
...
`@octocode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/octocode`.
...
- The package has no imports from `packages/octocode`, `packages/cli`, or
  `@octocode-ai/core`.
...
public `@octocode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/octocode`.
- Keep the legacy embedded worker and server startup in `packages/octocode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@octocode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/octocode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@octocode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@octocode-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for OctoCode domain state.
...
cd packages/octocode && bun typecheck
...
rg '@octocode-ai/core|packages/octocode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/octocode/src packages/octocode/test
```
...
- Build the current-platform `packages/octocode` binary.
