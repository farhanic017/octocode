# Remove `packages/octocode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/octocode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the octocode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/octocode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/octocode/src/account/repo.ts`
- `packages/octocode/src/cli/cmd/db.ts`
- `packages/octocode/src/cli/cmd/import.ts`
- `packages/octocode/src/cli/cmd/stats.ts`
- `packages/octocode/src/control-plane/workspace.ts`
- `packages/octocode/src/index.ts`
- `packages/octocode/src/node.ts`
- `packages/octocode/src/permission/index.ts`
- `packages/octocode/src/project/project.ts`
- `packages/octocode/src/server/projectors.ts`
- `packages/octocode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/octocode/src/server/shared/fence.ts`
- `packages/octocode/src/session/message-v2.ts`
- `packages/octocode/src/session/projectors.ts`
- `packages/octocode/src/session/prompt.ts`
- `packages/octocode/src/session/session.ts`
- `packages/octocode/src/session/todo.ts`
- `packages/octocode/src/share/share-next.ts`
- `packages/octocode/src/storage/db.ts`
- `packages/octocode/src/sync/index.ts`
- `packages/octocode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy octocode database wrapper; `packages/octocode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the octocode wrapper.
...
Status: Completed. `SyncEvent` and the octocode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy octocode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy octocode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy octocode database wrapper.
...
All migration groups are complete or superseded. `packages/octocode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No octocode data-migration group remains.

The previous octocode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate octocode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/octocode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/octocode/src`
- `bun typecheck` from `packages/octocode`
- Relevant package tests from `packages/octocode`, not the repo root
